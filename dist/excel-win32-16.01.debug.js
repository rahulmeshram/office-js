...
++scalarPropCount;
	...
	var LibraryBuilder=(function () {
		function LibraryBuilder(options) {
			...
			if (metadata.clientObjectTypes) {
				for (var i=0; i < metadata.clientObjectTypes.length; i++) {
					...
					if (Array.isArray(elem)) {
						this.ensureCompactJsonArraySize(elem, 11);
		...
		LibraryBuilder.prototype.ensureCompactJsonArraySize=function (value, size) {
		...
		LibraryBuilder.prototype.buildClientObjectType=function (typeInfo, fullyInitialize) {
			...
			if ((typeInfo.behaviorFlags & 32) !==0) {
				var func=this.getTypeOrFunction(LibraryBuilder.CustomizationCodeNamespace+"."+typeInfo.name+"_StaticCustomize");
				func.call(null, type);
			}
			if (fullyInitialize) {
		...
		LibraryBuilder.prototype.buildPrototype=function (type, typeInfo) {
			if (Array.isArray(typeInfo.scalarProperties)) {
				for (var i=0; i < typeInfo.scalarProperties.length; i++) {
					...
					if (Array.isArray(elem)) {
						this.ensureCompactJsonArraySize(elem, 5);
			...
			if (Array.isArray(typeInfo.navigationProperties)) {
				for (var i=0; i < typeInfo.navigationProperties.length; i++) {
					...
					if (Array.isArray(elem)) {
						this.ensureCompactJsonArraySize(elem, 7);
			...
			if (Array.isArray(typeInfo.scalarMethods)) {
				for (var i=0; i < typeInfo.scalarMethods.length; i++) {
					...
					if (Array.isArray(elem)) {
						this.ensureCompactJsonArraySize(elem, 6);
			...
			if (Array.isArray(typeInfo.navigationMethods)) {
				for (var i=0; i < typeInfo.navigationMethods.length; i++) {
					...
					if (Array.isArray(elem)) {
						this.ensureCompactJsonArraySize(elem, 8);
			...
			if ((propInfo.behaviorFlags & 2)===0) {
				descriptor.set=function (value) {
					if (propInfo.behaviorFlags & 4) {
						var customizationFunc=thisBuilder.getTypeOrFunction(LibraryBuilder.CustomizationCodeNamespace+"."+typeInfo.name+"_"+propInfo.originalName+"_Set");
			...
			var descriptor={
				get: function () {
					if (!this[thisBuilder.getFieldName(propInfo)]) {
						thisBuilder.throwIfApiNotSupported(typeInfo, propInfo);
						this[fieldName]=BatchApiHelper.createPropertyObject(thisBuilder.getTypeOrFunction(propInfo.propertyTypeFullName), this, propInfo.originalName, (propInfo.behaviorFlags & 16) !==0, propInfo.getMethodApiFlags);
					}
					if (propInfo.behaviorFlags & 64) {
						var customizationFunc=thisBuilder.getTypeOrFunction(LibraryBuilder.CustomizationCodeNamespace+"."+typeInfo.name+"_"+propInfo.originalName+"_Get");
		...
		LibraryBuilder.prototype.buildScalarMethod=function (type, typeInfo, methodInfo) {
			...
			type.prototype[methodInfo.name]=function () {
				...
				if (methodInfo.behaviorFlags & 1) {
					var customizationFunc=thisBuilder.getTypeOrFunction(LibraryBuilder.CustomizationCodeNamespace+"."+typeInfo.name+"_"+methodInfo.originalName);
					var applyArgs=[this];
					for (var i=0; i < args.length; i++) {
		...
		LibraryBuilder.prototype.buildNavigationMethod=function (type, typeInfo, methodInfo) {
			...
			type.prototype[methodInfo.name]=function () {
				...
				if ((methodInfo.behaviorFlags & 16) !==0) {
					return BatchApiHelper.createIndexerObject(thisBuilder.getTypeOrFunction(methodInfo.returnTypeFullName), this, args);
				}
				else {
					var operationType=0;
					if (methodInfo.behaviorFlags & 2) {
					...
					return BatchApiHelper.createMethodObject(thisBuilder.getTypeOrFunction(methodInfo.returnTypeFullName), this, methodInfo.originalName, operationType, args, (methodInfo.behaviorFlags & 4) !==0, (methodInfo.behaviorFlags & 8) !==0, methodInfo.returnObjectGetByIdMethodName, methodInfo.apiFlags);
				}
			};
		};
		LibraryBuilder.prototype.buildHandleResult=function (type, typeInfo) {
			...
			type.prototype._handleResult=function (value) {
				...
				if (typeInfo.behaviorFlags & 8) {
					var customizationFunc=thisBuilder.getTypeOrFunction(LibraryBuilder.CustomizationCodeNamespace+"."+typeInfo.name+"_HandleResult");
		...
		LibraryBuilder.prototype.buildHandleIdResult=function (type, typeInfo) {
			...
			type.prototype._handleIdResult=function (value) {
				...
				if (typeInfo.behaviorFlags & 16) {
					var customizationFunc=thisBuilder.getTypeOrFunction(LibraryBuilder.CustomizationCodeNamespace+"."+typeInfo.name+"_HandleIdResult");
					customizationFunc.call(this, this, value);
				}
				if (typeInfo.scalarProperties) {
					for (var i=0; i < typeInfo.scalarProperties.length; i++) {
				...
				get: function () {
					if (!this[fieldName]) {
						...
						if (evt.behaviorFlags & 2) {
							func=thisBuilder.getTypeOrFunction(LibraryBuilder.CustomizationCodeNamespace+"."+typeInfo.name+"_"+evt.name+"_EventArgsTransform");
						}
				...
				get: function () {
					if (!this[fieldName]) {
						...
						if (Utility.isNullOrEmptyString(evt.register)) {
							...
							var func_1=thisBuilder.getTypeOrFunction(evt.register);
							registerFunc=								function (handlerCallback) {
						...
						if (Utility.isNullOrEmptyString(evt.unregister)) {
							...
							var func_2=thisBuilder.getTypeOrFunction(evt.unregister);
							unregisterFunc=								function (handlerCallback) {
...
(function (Excel) {
	...
	var _libraryMetadata={ "version": "1.0.0",
		...
		"clientObjectTypes": [[1, 0, [["enableEvents", 1, 1]]],
			[2, 0, [["calculationMode", 1], [177, 3, 2], ["calculationState", 3, 2], ["decimalSeparator", 3, 3], ["thousandsSeparator", 3, 3], ["useSystemSeparators", 3, 3]], [["iterativeCalculation", 3, 35, 2, 0, 4], ["ribbon", 174, 35, 2, 0, 4], ["cultureInfo", 89, 3, 3, 0, 4]], [[178, 1], ["suspendApiCalculationUntilNextSync", 0, 1, 4], ["suspendScreenUpdatingUntilNextSync", 0, 0, 2]], [["createWorkbook", 6, 1, 10, 1, 0, 0, 179], [179, 6, 1, 2, 1, 4]]],
			[3, 0, [[180, 1], ["maxIteration", 1], ["maxChange", 1]]],
			[4, 0, [[181, 3, 5], [182, 3, 1], ["isDirty", 1, 2], ["use1904DateSystem", 1, 6], ["chartDataPointTrack", 1, 2], ["usePrecisionAsDisplayed", 1, 2], [177, 3, 2], ["autoSave", 3, 2], ["previouslySaved", 3, 2]], [["worksheets", 8, 19, 0, 0, 4], [183, 17, 19, 0, 0, 4], [184, 22, 19, 0, 0, 4], ["application", 2, 2, 0, 0, 4], ["bindings", 21, 19, 0, 0, 4], ["functions", 176, 2, 7, 0, 4], [94, 94, 2, 8, 0, 4], [185, 95, 19, 8, 0, 4], ["settings", 15, 19, 9, 0, 4], ["customXmlParts", 92, 19, 10, 0, 4], ["internalTest", 146, 2, 4, 0, 4], [186, 112, 35, 5, 0, 4], ["styles", 137, 19, 5, 0, 4], [187, 5, 3, 5, 0, 4], ["dataConnections", 152, 3, 5, 0, 4], ["_Runtime", 1, 2, 10, 0, 4], [188, 154, 19, 11, 0, 4], [189, 171, 19, 11, 0, 4], ["tableStyles", 138, 19, 11, 0, 4], ["pivotTableStyles", 140, 19, 11, 0, 4], ["slicerStyles", 142, 19, 11, 0, 4], ["timelineStyles", 144, 19, 11, 0, 4]], [["_RemoveReference", 1, 2], ["_GetObjectByReferenceId", 1, 2, 0, 4], ["_GetObjectTypeNameByReferenceId", 1, 2, 0, 4], ["_RemoveAllReferences", 0, 2], ["_GetReferenceCount", 0, 2, 0, 4], ["getIsActiveCollabSession", 0, 0, 2], ["registerCustomFunctions", 7, 0, 12, 8], ["_SetOsfControlContainerReadyForCustomFunctions", 0, 0, 12], ["close", 1, 0, 3], ["save", 1, 0, 3], [190, 0, 0, 2], [191, 0, 0, 2], [192, 0, 0, 13], [193, 0, 0, 13]], [["getSelectedRange", 11, 0, 10, 0, 4], ["getActiveCell", 11, 0, 10, 5, 4], ["_GetRangeForEventByReferenceId", 11, 1, 2, 0, 4], ["_GetRangeOrNullObjectForEventByReferenceId", 11, 1, 2, 0, 4], ["getActiveChart", 38, 0, 2, 2, 4], ["getActiveChartOrNullObject", 38, 0, 2, 2, 4], ["getSelectedRanges", 12, 0, 10, 2, 4], ["_GetRangesForEventByReferenceId", 12, 1, 2, 2, 4], ["_GetRangesOrNullObjectForEventByReferenceId", 12, 1, 2, 2, 4], ["getActiveSlicer", 170, 0, 2, 11, 4], ["getActiveSlicerOrNullObject", 170, 0, 2, 11, 4]], 0, 0, [["AutoSaveSettingChanged", 0, 2, "MessageType.workbookAutoSaveSettingChangedEvent", 194, 190, 191], [195, 3, 8, "_CC.office10EventIdDocumentSelectionChangedEvent", 196, 196, 196], ["WACOperationEvent", 2, 13, "MessageType.wacoperationEvent", 194, 192, 193], ["_Message", 3, 5, "_CC.office10EventIdRichApiMessageEvent", 196, 196, 196]]],
			[5, 0, [[197, 3]], 0, [[198, 1], [199, 1]]],
			[6, 0, [[200, 3]], 0, [["open", 0, 2, 0, 4]]],
			[7, 0, [[181, 1], [200, 3], [201, 1], ["visibility", 1], ["tabColor", 1, 5], ["standardWidth", 1, 5], ["standardHeight", 3, 5], ["showGridlines", 5, 1], ["showHeadings", 5, 1], ["enableCalculation", 1, 2]], [["charts", 37, 83, 0, 0, 4], [184, 22, 83, 0, 0, 4], [187, 9, 3, 7, 0, 4], [185, 95, 19, 8, 0, 4], [183, 17, 19, 9, 0, 4], ["freezePanes", 10, 3, 5, 0, 4], ["pageLayout", 147, 35, 2, 0, 4], ["visuals", 80, 83, 11, 0, 4], [202, 158, 19, 2, 0, 4], ["horizontalPageBreaks", 151, 19, 2, 0, 4], ["verticalPageBreaks", 151, 19, 2, 0, 4], [203, 88, 3, 2, 0, 4], [189, 171, 19
insert", 11, 1, 8], [300, 11, 0, 10, 0, 4], [301, 11, 0, 10, 0, 4], ["getOffsetRange", 11, 2, 10, 0, 4], ["getRow", 11, 1, 10, 0, 4], ["getColumn", 11, 1, 10, 0, 4], [302, 11, 1, 10, 0, 4], ["getBoundingRect", 11, 1, 10, 0, 4], ["getLastCell", 11, 0, 10, 0, 4], ["getLastColumn", 11, 0, 10, 0, 4], ["getLastRow", 11, 0, 10, 0, 4], [303, 11, 1, 10, 9, 4], ["getRowsAbove", 11, 1, 11, 8, 4], ["getRowsBelow", 11, 1, 11, 8, 4], ["getColumnsBefore", 11, 1, 11, 8, 4], ["getColumnsAfter", 11, 1, 11, 8, 4], ["getResizedRange", 11, 2, 11, 8, 4], ["getVisibleView", 13, 0, 2, 8, 4], [232, 11, 1, 10, 9, 4], ["getSurroundingRegion", 11, 0, 10, 5, 4], ["getAbsoluteResizedRange", 11, 2, 10, 5, 4], [304, 23, 1, 6, 2, 4], [305, 11, 2, 10, 2, 4], ["findOrNullObject", 11, 2, 10, 2, 4], ["removeDuplicates", 30, 2, 8, 2], [306, 12, 2, 10, 2, 4], [307, 12, 2, 10, 2, 4], ["getSpillingToRange", 11, 0, 10, 6, 4], ["getSpillParent", 11, 0, 10, 6, 4], ["getSpillingToRangeOrNullObject", 11, 0, 10, 6, 4], ["getSpillParentOrNullObject", 11, 0, 10, 6, 4], ["getMergedAreas", 12, 0, 10, 6, 4]]],
			[12, 2, [[273, 2], [270, 3], [271, 3], ["areaCount", 3], [272, 3], [276, 3], [277, 3], [279, 1]], [[308, 153, 19, 0, 0, 4], [287, 115, 19, 0, 0, 4], [284, 31, 35, 0, 0, 4], [288, 29, 35, 0, 0, 4], [285, 7, 2, 0, 0, 4]], [[291, 0, 2], [178], [289, 1], [296], [293, 4], [294, 2], [295], [290, 0, 2, 6]], [[300, 12, 0, 10, 0, 4], [301, 12, 0, 10, 0, 4], [302, 12, 1, 10, 0, 4], [303, 12, 1, 10, 0, 4], ["getOffsetRangeAreas", 12, 2, 10, 0, 4], ["getUsedRangeAreas", 12, 1, 10, 0, 4], ["getUsedRangeAreasOrNullObject", 12, 1, 10, 0, 4], [304, 23, 1, 6, 0, 4], [306, 12, 2, 10, 0, 4], [307, 12, 2, 10, 0, 4]]],
			[13, 0, [[260, 1], [262, 1], [263, 3], [264, 1], [265, 1], [275, 1], [274, 3], [268, 3], [269, 3], ["cellAddresses", 3], [309, 3]], [[310, 14, 19, 0, 0, 4]], 0, [[229, 11, 0, 10, 0, 4]]],
			[14, 1, 0, 0, [[247, 0, 2, 9, 4]], [[311, 13, 1, 2, 0, 4]], 0, 13],
			[15, 1, 0, 0, [[247, 0, 2, 0, 4]], [[252, 16, 1, 18, 0, 4], [253, 16, 2, 9], [254, 16, 1, 2, 0, 4]], 0, 16, [["SettingsChanged", 3, 0, "_CC.office10EventIdSettingsChangedEvent", 196, 196, 196]]],
			[16, 8, [[312, 3], [313, 5], [314, 2]], 0, [[204]]],
			[17, 1, 0, 0, [[247, 0, 2, 9, 4]], [[252, 18, 1, 18, 0, 4], [254, 18, 1, 2, 9, 4], [253, 18, 3, 8, 9], ["addFormulaLocal", 18, 3, 0, 9]], 0, 18],
			[18, 0, [[181, 3], [315, 3], [313, 3], [316, 1], [314, 2], ["comment", 1, 9], ["scope", 3, 9], [317, 1, 5]], [[285, 7, 2, 9, 0, 4], ["worksheetOrNullObject", 7, 2, 9, 0, 4], ["arrayValues", 19, 3, 5, 0, 4]], [[204, 0, 0, 9]], [[229, 11, 0, 10, 0, 4], [318, 11, 0, 10, 9, 4]]],
			[19, 0, [[262, 3], ["types", 3]]],
			[20, 0, [[200, 3], [315, 3]], 0, [["getText", 0, 2, 0, 4], [204, 0, 0, 8]], [["getTable", 24, 0, 2, 0, 4], [229, 11, 0, 2, 0, 4]], 0, 0, [["DataChanged", 3, 8, "_CC.office10EventIdBindingDataChangedEvent", 237, 196, 196], [195, 3, 8, "_CC.office10EventIdBindingSelectionChangedEvent", 237, 196, 196]]],
			[21, 1, [[319, 3]], 0, [[247, 0, 2, 9, 4]], [[252, 20, 1, 18, 0, 4], [311, 20, 1, 2, 0, 4], [253, 20, 3, 8, 8], ["addFromNamedItem", 20, 3, 0, 8], ["addFromSelection", 20, 2, 0, 8], [254, 20, 1, 2, 9, 4]], 0, 20],
			[22, 5, [[319, 3]], 0, [[247, 0, 2, 9, 4], [206, 0, 0, 5], [207, 0, 0, 5], [248, 0, 0, 2], [249, 0, 0, 2], [250, 0, 0, 2], [251, 0, 0, 2], [217, 0, 0, 6], [218, 0, 0, 6]], [[252, 24, 1, 18, 0, 4], [311, 24, 1, 2, 0, 4], [253, 24, 2, 8], [254, 24, 1, 2, 9, 4]], 0, 24, [[256, 0, 2, "MessageType.tableAddedEvent", 320, 248, 249], [239, 2, 5, "MessageType.tableDataChangedEvent", 320, 206, 207], [257, 0, 2, "MessageType.tableDeletedEvent", 320, 250, 251], [242, 0, 6, "MessageType.tableFilteredEvent", 320, 217, 218]]],
			[23, 1, 0, 0, [[247, 0, 2, 0, 4]], [[255, 24, 0, 10, 0, 4], [252, 24, 1, 18, 0, 4]], 0, 24],
			[24, 24, [[200, 3], [181, 1], ["showHeaders", 1], ["showTotals", 1], [279, 1], ["highlightFirstColumn", 1, 8], ["highlightLastColumn", 1, 8], ["showBandedRows", 1, 8], ["showBandedColumns", 1, 8], ["showFilterButton", 1, 8], ["legacyId", 3, 1]], [[321, 25, 19, 0, 0, 4], [310, 27, 19, 0, 0, 4], [286, 86, 3, 7, 0, 4], [285, 7, 2, 7, 0, 4], [203, 88, 3, 2, 0, 4], ["tableStyle", 139, 35, 15, 0, 4]], [[204], [322, 0, 0, 7], ["reapplyFilters", 0, 0, 7], [212, 0, 0, 5], [213, 0, 0, 5], [206, 0, 0, 5], [207, 0, 0, 5], ["clearStyle", 0, 0, 6], [217, 0, 0, 6], [218, 0, 0, 6], [323, 1, 0, 15]], [[229, 11, 0, 10, 0, 4], [324, 11, 0, 10, 0, 4], [325, 11, 0, 10, 0, 4], [326, 11, 0, 10, 0, 4], ["convertToRange", 11, 0, 8, 7]], "workbook.tables", 0, [[239, 2, 5, "MessageType.tableDataChangedEvent", 327, 206, 207], [242, 0, 6, "MessageType.tableFilteredEvent", 327, 217, 218], [195, 2, 5, "MessageType.tableSelectionChangedEvent", 237, 212, 213]]],
			[25, 1, [[319, 3]], 0, [[247, 0, 2, 9, 4]], [[252, 26, 1, 18, 0, 4], [311, 26, 1, 2, 0, 4], [253, 26, 3, 8], [254, 26, 1, 2, 9, 4]], 0, 26],
			[26, 0, [[200, 3], [309, 3], [262, 1], [181, 1]], [["filter", 87, 3, 7, 0, 4]], [[204]], [[229, 11, 0, 10, 0, 4], [324, 11, 0, 10, 0, 4], [325, 11, 0, 10, 0, 4], [326, 11, 0, 10, 0, 4]]],
			[27, 1, [[319, 3]], 0, [[247, 0, 2, 9, 4]], [[311, 28, 1, 2, 0, 4], [253, 28, 2, 8]], 0, 28],
			[28, 0, [[309, 3], [262, 1]], 0, [[204]], [[229, 11, 0, 10, 0, 4]]],
			[29, 0, [[315, 3], [328, 1], ["prompt", 1], ["errorAlert", 1], ["ignoreBlanks", 1], ["valid", 3]], 0, [[289]], [["getInvalidCells", 12, 0, 10, 2, 4], ["getInvalidCellsOrNullObject", 12, 0, 10, 2, 4]]],
			[30, 0, [["removed", 3], ["uniqueRemaining", 3]]],
			[31, 0, [[329, 1], [330, 1], [331, 1], ["columnWidth", 1, 7], [332, 1, 7], [333, 1, 5], ["useStandardHeight", 1, 5], ["useStandardWidth", 1, 5], [334, 1, 2], [335, 1, 2], [336, 1, 2], [337, 1, 2]], [[338, 33, 35, 0, 0, 4], [339, 36, 35, 0, 0, 4], [340, 35, 51, 0, 0, 4], [187, 32, 35, 7, 0, 4]], [["autofitColumns", 0, 0, 7], ["autofitRows", 0, 0, 7], ["adjustIndent", 1, 0, 14]]],
			[32, 0, [[341, 1], [342, 1]]],
			[33, 0, [[343, 1], [344, 1, 2], ["patternTintAndShade", 1, 2], ["pattern", 1, 2], ["patternColor", 1, 2]], 0, [[289]]],
			[34, 0, [[345, 3], [279, 1], [346, 1], [343, 1], [344, 1, 2]]],
			[35, 1, [[319, 3], [344, 1, 2]], 0, 0, [[252, 34, 1, 18, 0, 4], [311, 34, 1, 2, 0, 4]], 0, 34],
			[36, 0, [[181, 1], [347, 1], [343, 1], [348, 1], [349, 1], [350, 1], [351, 1, 2], ["subscript", 1, 2], ["superscript", 1, 2], [344, 1, 2]]],
			[37, 5, [[319, 3]], 0, [[247, 0, 2, 9, 4], [248, 0, 0, 1], [249, 0, 0, 1], [208, 0, 0, 1], [209, 0, 0, 1], [210, 0, 0, 1], [211, 0, 0, 1], [250, 0, 0, 1], [251, 0, 0, 1]], [[253, 38, 3, 9], [311, 38, 1, 2, 0, 4], [352, 38, 1, 18, 0, 4], [252, 38, 1, 2, 0, 4], [254, 38, 1, 2, 9, 4]], 0, 38, [[236, 0, 1, "MessageType.chartActivatedEvent", 353, 208, 209], [256, 0, 1, "MessageType.chartAddedEvent", 353, 248, 249], [241, 0, 1, "MessageType.chartDeactivatedEvent", 353, 210, 211], [257, 0, 1, "MessageType.chartDeletedEvent", 353, 250, 251]]],
			[38, 0, [[181, 1], [280, 1], [281, 1], [283, 1], [282, 1], [200, 3, 5], ["showAllFieldButtons", 1, 5], [354, 1, 5], ["showDataLabelsOverMaximum", 1, 1], ["categoryLabelLevel", 1, 1], [279, 1, 1], ["displayBlanksAs", 1, 1], ["plotBy", 1, 1], ["plotVisibleOnly", 1, 1], ["seriesNameLevel", 1, 1]], [[355, 64, 35, 0, 0, 4], [356, 52, 35, 0, 0, 4], ["legend", 59, 35, 0, 0, 4], ["series", 41, 19, 0, 0, 4], ["axes", 47, 35, 0, 0, 4], [284, 40, 35, 0, 0, 4], [285, 7, 2, 7, 0, 4], ["plotArea", 78, 35, 1, 0, 4], ["pivotOptions", 39, 35, 2, 0, 4]], [["setData", 2, 1], [204], ["setPosition", 2], [292, 3, 2, 7, 4], [208, 0, 0, 1], [209, 0, 0, 1], [210, 0, 0, 1], [211, 0, 0, 1], [205, 0, 2, 2]], 0, 0, 0, [[236, 0, 1, "MessageType.chartActivatedEvent", 237, 208, 209], [241, 0, 1, "MessageType.chartDeactivatedEvent", 237, 210, 211]]],
			[39, 0, [["showAxisFieldButtons", 1], ["showLegendFieldButtons", 1], ["showReportFilterFieldButtons", 1], ["showValueFieldButtons", 1]]],
			[40, 0, [["roundedCorners", 1, 2], ["colorScheme", 1, 2]], [[338, 67, 3, 0, 0, 4], [339, 72, 35, 0, 0, 4], [357, 68, 35, 5, 0, 4]]],
			[41, 1, [[319, 3]], 0, [[247, 0, 2, 9, 4]], [[311, 42, 1, 2, 0, 4], [253, 42, 2, 8, 5]], 0, 42],
			[42, 0, [[181, 1], [354, 1, 5], ["hasDataLabels", 1, 5], ["filtered", 1, 5], [358, 1, 5], [359, 1, 5], [360, 1, 5], [361, 1, 5], [362, 1, 5], ["smooth", 1, 5], ["plotOrder", 1, 5], ["gapWidth", 1, 5], ["doughnutHoleSize", 1, 5], [363, 1, 1], ["explosion", 1, 1], ["firstSliceAngle", 1, 1], ["invertIfNegative", 1, 1], ["bubbleScale", 1, 2], ["secondPlotSize", 1, 1], ["splitType", 1, 1], ["splitValue", 1, 2], ["varyByCategories", 1, 1], ["showLeaderLines", 1, 2], ["overlap", 1, 1], ["gradientStyle", 1, 2], ["gradientMinimumType", 1, 2], ["gradientMidpointType", 1, 2], ["gradientMaximumType", 1, 2], ["gradientMinimumValue", 1, 2], ["gradientMidpointValue", 1, 2], ["gradientMaximumValue", 1, 2], ["gradientMinimumColor", 1, 2], ["gradientMidpointColor", 1, 2], ["gradientMaximumColor", 1, 2], ["parentLabelStrategy", 1, 2], ["showConnectorLines", 1, 2], ["invertColor", 1, 2]], [["points", 44, 19, 0, 0, 4], [284, 43, 35, 0, 0, 4], ["trendlines", 74, 19, 5, 0, 4], ["xerrorBars", 55, 35, 2, "XErrorBars", 4], ["yerrorBars", 55, 35, 2, "YErrorBars", 4], [356, 52, 35, 1, 0, 4], ["binOptions", 69, 35, 2, 0, 4], ["mapOptions", 63, 35, 2, 0, 4], ["boxwhiskerOptions", 70, 35, 2, 0, 4]], [[204, 0, 0, 5], ["setXAxisValues", 1, 0, 5], ["setValues", 1, 0, 5], ["setBubbleSizes", 1, 0, 5], ["getDimensionValues", 1, 0, 6]]],
			[54, 0, 0, [[339, 72, 35, 0, 0, 4], [338, 67, 3, 0, 0, 4], [357, 68, 35, 1, 0, 4]]],
			[55, 0, [["endStyleCap", 1], ["include", 1], [315, 1], [316, 1]], [[284, 56, 35, 0, 0, 4]]],
			[56, 0, 0, [[364, 71, 35, 0, 0, 4]]],
			[57, 0, [[316, 1]], [[284, 58, 35, 0, 0, 4]]],
			[58, 0, 0, [[364, 71, 35, 0, 0, 4]]],
			[59, 0, [[316, 1], [201, 1], [375, 1], [281, 1, 5], [280, 1, 5], [283, 1, 5], [282, 1, 5], [360, 1, 5]], [[284, 62, 35, 0, 0, 4], ["legendEntries", 61, 19, 5, 0, 4]]],
			[60, 0, [[316, 1], [281, 3, 1], [280, 3, 1], [283, 3, 1], [282, 3, 1], [309, 3, 1]]],
			[61, 1, 0, 0, [[247, 0, 2, 0, 4]], [[311, 60, 1, 2, 0, 4]], 0, 60],
			[62, 0, 0, [[339, 72, 35, 0, 0, 4], [338, 67, 3, 0, 0, 4], [357, 68, 35, 1, 0, 4]]],
			[63, 0, [[376, 1], ["labelStrategy", 1], ["projectionType", 1]]],
			[64, 0, [[316, 1], [263, 1], [375, 1], [330, 1, 5], [280, 1, 5], [281, 1, 5], [283, 3, 5], [282, 3, 5], [331, 1, 5], [333, 1, 5], [201, 1, 5], [360, 1, 5]], [[284, 66, 35, 0, 0, 4]], [[366, 1, 0, 5]], [[377, 65, 2, 2, 5, 4]]],
			[65, 0, 0, [[339, 72, 35, 0, 0, 4]]],
			[66, 0, 0, [[339, 72, 35, 0, 0, 4], [338, 67, 3, 0, 0, 4], [357, 68, 35, 5, 0, 4]]],
			[67, 4, 0, 0, [[378, 1], [289]]],
			[68, 0, [[343, 1], [379, 1], [346, 1]], 0, [[289, 0, 0, 1]]],
			[69, 0, [[315, 1], [283, 1], [319, 1], ["allowOverflow", 1], ["allowUnderflow", 1], ["overflowValue", 1], ["underflowValue", 1]]],
			[70, 0, [["showInnerPoints", 1], ["showOutlierPoints", 1], ["showMeanMarker", 1], ["showMeanLine", 1], ["quartileCalculation", 1]]],
			[71, 0, [[343, 1], [379, 1, 5], [346, 1, 5]], 0, [[289]]],
			[72, 0, [[349, 1], [343, 1], [348, 1], [181, 1], [347, 1], [350, 1]]],
			[73, 0, [[315, 1], ["polynomialOrder", 1], ["movingAveragePeriod", 1], [314, 2], ["showEquation", 1, 1], ["showRSquared", 1, 1], ["forwardPeriod", 1, 1], ["backwardPeriod", 1, 1], [181, 1], ["intercept", 1]], [[284, 75, 35, 0, 0, 4], ["label", 76, 35, 1, 0, 4]], [[204]]],
			[74, 1, 0, 0, [[247, 0, 2, 0, 4]], [[253, 73, 1, 8], [252, 73, 1, 18, 0, 4]], 0, 73],
			[75, 0, 0, [[364, 71, 35, 0, 0, 4]]],
			[76, 0, [[280, 1], [281, 1], [283, 3], [282, 3], [317, 1], [333, 1], [330, 1], [331, 1], [263, 1], [374, 1], [260, 1], [365, 1, 2]], [[284, 77, 35, 0, 0, 4]]],
			[77, 0, 0, [[338, 67, 3, 0, 0, 4], [357, 68, 35, 0, 0, 4], [339, 72, 35, 0, 0, 4]]],
			[78, 0, [[281, 1], [280, 1], [283, 1], [282, 1], ["insideLeft", 1], ["insideTop", 1], ["insideWidth", 1], ["insideHeight", 1], [201, 1]], [[284, 79, 35, 0, 0, 4]]],
			[79, 0, 0, [[357, 68, 35, 0, 0, 4], [338, 67, 3, 0, 0, 4]]],
			[80, 5, 0, 0, [["getDefinitions", 0, 2, 0, 4], ["getPreview", 4, 2, 0, 4], ["bootstrapAgaveVisual", 0, 0, 0, 2], [247, 0, 2, 0, 4], [212, 0, 2], [213, 0, 2]], [[253, 81, 3, 8, 0, 2], [352, 81, 1, 18, 0, 4], ["getSelectedOrNullObject", 81, 0, 2, 0, 4]], 0, 81, [["AgaveVisualUpdate", 2, 0, "MessageType.agaveVisualUpdateEvent", 196, 380, 380], [195, 0, 0, "MessageType.visualSelectionChangedEvent", 353, 212, 213]]],
			[81, 0, [[200, 3], ["isSupportedInVisualTaskpane", 3]], [[186, 83, 18, 0, 0, 4]], [[204, 0, 0, 0, 2], ["getProperty", 1, 2, 0, 4], ["setProperty", 2, 0, 0, 2], ["changeDataSource", 2, 0, 0, 2], ["getDataSource", 0, 2, 0, 4], ["setPropertyToDefault", 1, 0, 0, 2], [381, 0, 2], [382, 0, 2], ["serializeProperties", 0, 2, 0, 4], ["deserializeProperties", 1]], [["getChildProperties", 83, 2, 6, 0, 4], ["getDataControllerClient", 84, 0, 2, 0, 4], ["getElementChildProperties", 83, 3, 6, 0, 4]], 0, 0, [["ChangeNotification", 2, 0, "MessageType.visualChangeEvent", 237, 381, 382]]],
			[82, 0, [[315, 3], [313, 3], [200, 3], ["localizedName", 3], [258, 3], ["localizedOptions", 3], ["hasDefault", 3], ["isDefault", 3], [383, 3], [384, 3], ["stepSize", 3], ["hideMeButShowChildrenUI", 3], ["expandableUI", 3], ["nextPropOnSameLine", 3], ["showResetUI", 3]], 0, [["getBoolMetaProperty", 1, 2, 0, 4]]],
			[83, 1, 0, 0, [[247, 0, 2, 0, 4]], [[252, 82, 1, 18, 0, 4], [311, 82, 1, 2, 0, 4]], 0, 82],
			[84, 0, 0, 0, [["getWells", 0, 2, 0, 4], ["getAssociatedFields", 1, 2, 0, 4], ["getAvailableFields", 1, 2, 0, 4], ["addField", 3, 0, 0, 2], ["removeField", 2, 0, 0, 2], ["moveField", 3, 0, 0, 2]]],
			[85, 0, 0, 0, [[385, 5]]],
			[86, 0, [["matchCase", 3], ["method", 3], [386, 3]], 0, [[385, 3], [289], [387]]],
			[87, 0, [[388, 3]], 0, [[385, 1], [289], ["applyBottomItemsFilter", 1], ["applyBottomPercentFilter", 1], ["applyCellColorFilter", 1], ["applyDynamicFilter", 1], ["applyFontColorFilter", 1], ["applyValuesFilter", 1], ["applyTopItemsFilter", 1], ["applyTopPercentFilter", 1], ["applyIconFilter", 1], ["applyCustomFilter", 3]]],
			[88, 0, [[180, 3], ["isDataFiltered", 3], [388, 3]], 0, [[385, 3], [387], [389], ["clearCriteria"]], [[229, 11, 0, 10, 0, 4], [318, 11, 0, 10, 0, 4]]],
			[89, 0, [[181, 3]], [[260, 90, 3, 0, 0, 4]]],
			[90, 0, [["numberDecimalSeparator", 3], ["numberGroupSeparator", 3]]],
			[91, 1, 0, 0, [[247, 0, 2, 0, 4]], [[252, 93, 1, 18, 0, 4], [254, 93, 1, 2, 0, 4], ["getOnlyItem", 93, 0, 2, 0, 4], ["getOnlyItemOrNullObject", 93, 0, 2, 0, 4]], 0, 93],
			[92, 1, 0, 0, [[247, 0, 2, 0, 4]], [[252, 93, 1, 18, 0, 4], [253, 93, 1, 8], ["getByNamespace", 91, 1, 6, 0, 4], [254, 93, 1, 2, 0, 4]], 0, 93],
			[93, 0, [[200, 3], ["namespaceUri", 3]], 0, [[204], ["getXml", 0, 2, 0, 4], ["setXml", 1]]],
			[94, 0, 0, 0, [["bindingGetData", 1, 2, 0, 4], ["getSelectedData", 1, 2, 0, 4], ["gotoById", 1, 2, 0, 4], ["bindingAddFromSelection", 1, 2], ["bindingGetById", 1, 2, 0, 4], ["bindingReleaseById", 1, 2], ["bindingGetAll", 0, 2, 0, 4], ["bindingAddFromNamedItem", 1, 2], ["bindingAddFromPrompt", 1, 2], ["bindingDeleteAllDataValues", 1], ["setSelectedData", 1], ["bindingClearFormats", 1], ["bindingSetData", 1], ["bindingSetFormats", 1], ["bindingSetTableOptions", 1], ["bindingAddRows", 1], ["bindingAddColumns", 1], ["getFilePropertiesAsync", 0, 2, 4, 4]]],
			[95, 1, 0, 0, [[390], [247, 0, 2, 9, 4]], [[252, 96, 1, 18, 0, 4], [254, 96, 1, 2, 9, 4], [253, 96, 3, 8, 1]], 0, 96],
			[96, 0, [[181, 1], [200, 3, 10], ["useCustomSortLists", 1, 2], ["enableDataValueEditing", 1, 2]], [[285, 7, 2, 0, 0, 4], ["hierarchies", 98, 19, 1, 0, 4], ["rowHierarchies", 100, 19, 1, 0, 4], ["columnHierarchies", 100, 19, 1, 0, 4], ["dataHierarchies", 104, 19, 1, 0, 4], ["filterHierarchies", 102, 19, 1, 0, 4], ["layout", 97, 2, 1, 0, 4]], [["refresh"], [204, 0, 0, 1]]],
			[97, 0, [["showColumnGrandTotals", 1], ["showRowGrandTotals", 1], ["enableFieldList", 1, 11], ["subtotalLocation", 1], ["layoutType", 1], ["autoFormat", 1, 2], ["preserveFormatting", 1, 2]], [["pivotStyle", 141, 35, 15, 0, 4]], [["getPivotItems", 2, 0, 2], ["setAutoSortOnCell", 2, 0, 2], [323, 1, 0, 15]], [[229, 11], ["getRowLabelRange", 11], ["getColumnLabelRange", 11], ["getFilterAxisRange", 11], [325, 11], [231, 11, 3, 0, 6], ["getDataHierarchy", 105, 1, 0, 2]]],
			[98, 1, 0, 0, [[247, 0, 2, 0, 4]], [[252, 99, 1, 18, 0, 4], [254, 99, 1, 2, 0, 4]], 0, 99],
			[99, 0, [[200, 3], [181, 1]], [[386, 106, 19, 0, 0, 4]]],
			[100, 1, 0, 0, [[247, 0, 2, 0, 4], [389, 1]], [[252, 101, 1, 18, 0, 4], [254, 101, 1, 2, 0, 4], [253, 101, 1, 8]], 0, 101],
			[101, 0, [[200, 3], [181, 1], [201, 1]], [[386, 106, 19, 0, 0, 4]], [[391]]],
			[102, 1, 0, 0, [[247, 0, 2, 0, 4], [389, 1]], [[252, 103, 1, 18, 0, 4], [254, 103, 1, 2, 0, 4], [253, 103, 1, 8]], 0, 103],
			[103, 0, [[200, 3], [181, 1], [201, 1], ["enableMultipleFilterItems", 1]], [[386, 106, 19, 0, 0, 4]], [[391]]],
			[104, 1, 0, 0, [[247, 0, 2, 0, 4], [389, 1]], [[252, 105, 1, 18, 0, 4], [254, 105, 1, 2, 0, 4], [253, 105, 1, 8]], 0, 105],
			[105, 0, [[200, 3], [181, 1], [201, 1], [260, 1], ["summarizeBy", 1], ["showAs", 1]], [["field", 107, 35, 0, 0, 4]], [[391]]],
			[106, 1, 0, 0, [[247, 0, 2, 0, 4]], [[252, 107, 1, 18, 0, 4], [254, 107, 1, 2, 0, 4]], 0, 107],
			[107, 0, [[200, 3], [181, 1], ["subtotals", 1], ["showAllItems", 1]], [["items", 108, 19, 0, 0, 4]], [["sortByLabels", 1, 1], ["sortByValues", 3, 0, 2]]],
			[108, 1, 0, 0, [[247, 0, 2, 0, 4]], [[252, 109, 1, 18, 0, 4], [254, 109, 1, 2, 0, 4]], 0, 109],
			[109, 0, [[200, 3], [181, 1], ["isExpanded", 1], [316, 1]]],
			[110, 0, [[312, 3], [313, 3]]],
			[111, 1, 0, 0, [[247, 0, 2, 0, 4]], [[252, 110, 1, 18, 0, 4], [254, 110, 1]], 0, 110],
			[112, 0, [[355, 1], ["subject", 1], ["author", 1], ["keywords", 1], [188, 1], ["lastAuthor", 3], ["revisionNumber", 1], [392, 11], ["category", 1], ["manager", 1], ["company", 1]], [[393, 114, 19, 0, 0, 4]]],
			[113, 0, [[312, 3], [313, 1], [315, 3]], 0, [[204]]],
			[114, 1, 0, 0, [[247, 0, 2, 0, 4], ["deleteAll"]], [[254, 113, 1, 2, 0, 4], [253, 113, 2, 8], [252, 113, 1, 18, 0, 4]], 0, 113],
			[115, 1, 0, 0, [[247, 0, 2, 0, 4], ["clearAll"]], [[311, 116, 1, 2, 0, 4], [253, 116, 1, 8], [252, 116, 1, 18, 0, 4]], 0, 116],
			[116, 0, [["stopIfTrue", 1], ["priority", 1], [315, 3], [200, 3]], [["dataBarOrNullObject", 117, 35, 0, 0, 4], ["dataBar", 117, 35, 0, 0, 4], ["customOrNullObject", 120, 35, 0, 0, 4], [393, 120, 35, 0, 0, 4], ["iconSet", 122, 35, 0, 0, 4], ["iconSetOrNullObject", 122, 35, 0, 0, 4], ["colorScale", 123, 35, 0, 0, 4], ["colorScaleOrNullObject", 123, 35, 0, 0, 4], ["topBottom", 124, 35, 0, 0, 4], ["topBottomOrNullObject", 124, 35, 0, 0, 4], ["preset", 125, 35, 0, 0, 4], ["presetOrNullObject", 125, 35, 0, 0, 4], ["textComparison", 126, 35, 0, 0, 4], ["textComparisonOrNullObject", 126, 35, 0, 0, 4], ["cellValue", 127, 35, 0, 0, 4], ["cellValueOrNullObject", 127, 35, 0, 0, 4]], [[204]], [[229, 11, 0, 10, 0, 4], [318, 11, 0, 10, 0, 4], [235, 12, 0, 10, 2, 4]]],
			[117, 0, [["showDataBarOnly", 1], ["barDirection", 1], ["axisFormat", 1], ["axisColor", 1], ["lowerBoundRule", 1], ["upperBoundRule", 1]], [["positiveFormat", 118, 35, 0, 0, 4], ["negativeFormat", 119, 35, 0, 0, 4]]],
			[118, 0, [[394, 1], ["gradientFill", 1], [395, 1]]],
			[119, 0, [[394, 1], ["matchPositiveFillColor", 1], [395, 1], ["matchPositiveBorderColor", 1]]],
			[120, 0, 0, [[328, 121, 35, 0, 0, 4], [284, 128, 35, 0, 0, 4]]],
			[121, 0, [[317, 1], ["formulaLocal", 1], ["formulaR1C1", 1]]],
			[122, 0, [["reverseIconOrder", 1], ["showIconOnly", 1], [279, 1], [388, 1]]],
			[123, 0, [["threeColorScale", 3], [388, 1]]],
			[124, 0, [[328, 1]], [[284, 128, 35, 0, 0, 4]]],
			[125, 0, [[328, 1]], [[284, 128, 35, 0, 0, 4]]],
			[126, 0, [[328, 1]], [[284, 128, 35, 0, 0, 4]]],
			[127, 0, [[328, 1]], [[284, 128, 35, 0, 0, 4]]],
			[128, 0, [[260, 1]], [[338, 130, 35, 0, 0, 4], [339, 129, 35, 0, 0, 4], [340, 132, 51, 0, 0, 4]]],
			[129, 0, [[343, 1], [348, 1], [349, 1], [350, 1], [351, 1]], 0, [[289]]],
			[130, 0, [[343, 1]], 0, [[289]]],
			[131, 0, [[345, 3], [279, 1], [343, 1]]],
			[132, 1, [[319, 3]], [[280, 131, 35, 0, 0, 4], ["bottom", 131, 35, 0, 0, 4], [281, 131, 35, 0, 0, 4], [396, 131, 35, 0, 0, 4]], 0, [[252, 131, 1, 18, 0, 4], [311, 131, 1, 2, 0, 4]], 0, 131],
			[133, 0, 0, 0, 0, [["getFormatter", 134, 1]], 0, 0, 0, "Microsoft.ExcelServices.NumberFormattingService", 4],
			[134, 0, [["isDateTime", 3], ["isPercent", 3], ["isCurrency", 3], ["isNumeric", 3], [397, 3], ["hasYear", 3], ["hasMonth", 3], ["hasDayOfWeek", 3]], 0, [[284, 1]]],
			[135, 36, [["status", 2]], 0, [["register", 2]], 0, 0, 0, 0, "Microsoft.ExcelServices.CustomFunctionManager", 4],
			[136, 0, [["builtIn", 3], [342, 1], [330, 1], ["includeAlignment", 1], ["includeBorder", 1], ["includeFont", 1], ["includeNumber", 1], ["includePatterns", 1], ["includeProtection", 1], [336, 1], [341, 1], [181, 3], [260, 1], [261, 1], [334, 1], [335, 1], [331, 1], [329, 1], [333, 5, 1], [337, 1, 1]], [[340, 35, 51, 0, 0, 4], [339, 36, 35, 0, 0, 4], [338, 33, 35, 0, 0, 4]], [[204]]],
			[137, 1, 0, 0, [[253, 1], [247, 0, 2, 2, 4]], [[252, 136, 1, 18, 0, 4], [311, 136, 1, 2, 2, 4]], 0, 136],
, 4], [298, 162, 2, 0, 0, 4], ["parentGroup", 159, 2, 0, 0, 4], [364, 164, 2, 0, 0, 4], ["lineFormat", 166, 35, 0, 0, 4]], [["setZOrder", 1], ["incrementLeft", 1], ["incrementTop", 1], ["incrementRotation", 1], ["scaleHeight", 3], ["scaleWidth", 3], [204], ["getAsImage", 1], [208], [209], [210], [211]], [["copyTo", 159, 1, 8, 11, 0, 0, 422], [422, 159, 1, 2, 0, 4]], 0, 0, [[236, 0, 0, "MessageType.shapeActivatedEvent", 237, 208, 209], [241, 0, 0, "MessageType.shapeDeactivatedEvent", 237, 210, 211]]],
			[160, 0, [[200, 3]], [[423, 159, 2, 0, 0, 4]]],
			[161, 0, [[200, 3], [284, 3, 0, 284]], [[423, 159, 2, 0, 0, 4]]],
			[162, 0, [[200, 3]], [[202, 163, 19, 0, 0, 4], [423, 159, 2, 0, 0, 4]], [[299]]],
			[163, 1, 0, 0, [[247, 0, 2, 0, 4]], [[352, 159, 1, 18, 0, 4], [311, 159, 1, 2, 0, 4], [252, 159, 1, 2, 0, 4]], 0, 159],
			[164, 0, [[200, 3], [424, 1, 0, 424], ["beginArrowheadLength", 1], ["beginArrowheadStyle", 1], ["beginArrowheadWidth", 1], ["endArrowheadLength", 1], ["endArrowheadStyle", 1], ["endArrowheadWidth", 1], ["isBeginConnected", 3], ["beginConnectedSite", 3], ["isEndConnected", 3], ["endConnectedSite", 3]], [[423, 159, 2, 0, 0, 4], ["beginConnectedShape", 159, 2, 0, 0, 4], ["endConnectedShape", 159, 2, 0, 0, 4]], [["connectBeginShape", 2], ["disconnectBeginShape"], ["connectEndShape", 2], ["disconnectEndShape"]]],
			[165, 0, [["foregroundColor", 1], [315, 3], [425, 1]], 0, [[289], [378, 1]]],
			[166, 0, [[316, 1], [343, 1], [279, 1], [346, 1], ["dashStyle", 1], [425, 1]]],
			[167, 0, [[410, 1], [411, 1], [412, 1], [413, 1], [330, 1], ["horizontalOverflow", 1], [331, 1], ["verticalOverflow", 1], [409, 1], [334, 1], ["hasText", 3], ["autoSizeSetting", 1]], [["textRange", 168, 2, 0, 0, 4]], [["deleteText"]]],
			[168, 0, [[263, 1]], [[339, 169, 35, 0, 0, 4]], 0, [[377, 168, 2]]],
			[169, 0, [[347, 1], [181, 1], [343, 1], [349, 1], [348, 1], [350, 1]]],
			[170, 0, [[200, 3], [181, 1], ["caption", 1], [281, 1], [280, 1], [283, 1], [282, 1], ["nameInFormula", 1, 3], ["isFilterCleared", 3], [279, 1], ["sortBy", 1], ["sortUsingCustomLists", 1, 15], [269, 1, 15], ["disableMoveResizeUI", 1, 15], ["displayHeader", 1, 15], [332, 1, 15]], [["slicerItems", 173, 19, 0, 0, 4], [285, 7, 35, 0, 0, 4], ["slicerStyle", 143, 35, 15, 0, 4]], [[204], [322], ["getSelectedItems"], ["selectItems", 1], [205, 0, 2, 15], [323, 1, 0, 15]]],
			[171, 1, 0, 0, [[247, 0, 2, 0, 4]], [[253, 170, 3, 8], [311, 170, 1, 2, 0, 4], [252, 170, 1, 18, 0, 4], [254, 170, 1, 2, 0, 4]], 0, 170],
...
!function (modules) {
	...
	__webpack_require__.m=modules, __webpack_require__.c=installedModules, __webpack_require__.d=function (exports, name, getter) {
		...
		Object.defineProperty(exports, "__esModule", {
			value: !0
		}), __webpack_require__(2), "undefined"==typeof CustomFunctionMappings && (window.CustomFunctionMappings={}),
			"undefined"==typeof Promise && (window.Promise=Office.Promise), window.OfficeExtensionBatch=window.OfficeExtensionBatch || window.OfficeExtension;
		var customfunctions_agave_initialize_1=__webpack_require__(3);
		!function () {
			var shouldInitialize=!1;
			"undefined" !=typeof OSF && OSF._OfficeAppFactory && OSF._OfficeAppFactory.getHostInfo && (0 !=(2 & (OSF._OfficeAppFactory && OSF._OfficeAppFactory.getHostInfo().flags)) && (shouldInitialize=!0));
			customfunctions_agave_initialize_1["default"](shouldInitialize);
		}();
	}, function (module, exports) {
		!function (self) {
			"use strict";
			if (!self.fetch) {
				...
				if (support.arrayBuffer)
					var viewClasses=["[object Int8Array]", "[object Uint8Array]", "[object Uint8ClampedArray]", "[object Int16Array]", "[object Uint16Array]", "[object Int32Array]", "[object Uint32Array]", "[object Float32Array]", "[object Float64Array]"], isDataView=function (obj) {
			...
			function Request(input, options) {
				...
				if (this.credentials=options.credentials || this.credentials || "omit", !options.headers && this.headers || (this.headers=new Headers(options.headers)),
					...
					this.referrer=null, ("GET"===this.method || "HEAD"===this.method) && body)
		...
		var CFRuntime=__webpack_require__(4);
		...
		var OfficeExtension=__webpack_require__(0), Core=__webpack_require__(0), _createPropertyObject=OfficeExtension.BatchApiHelper.createPropertyObject, _createRootServiceObject=(OfficeExtension.BatchApiHelper.createMethodObject,
		...
		var CustomFunctionLoggingSeverity, InvocationContext=function () {
			...
			return Object.defineProperty(InvocationContext.prototype, "onCanceled", {
				get: function () {
					if (!_isNullOrUndefined(this._onCanceled) && "function"==typeof this._onCanceled)
			...
			return CustomFunctionsLogger.logEvent=function (log, data, data2) {
				if (CustomFunctionsLogger.s_shouldLog || OfficeExtension.CoreUtility._logEnabled) {
					...
					if (data2 && (logMessage=logMessage+" "+data2), OfficeExtension.Utility.log(logMessage),
						CustomFunctionsLogger.s_shouldLog)
						switch (log.Severity) {
		...
		!function (ErrorCode) {
			ErrorCode.invalidValue="#VALUE!", ErrorCode.notAvailable="#N/A", ErrorCode.divisionByZero="#DIV/0!",
				ErrorCode.invalidNumber="#NUM!", ErrorCode.nullReference="#NULL!";
		}(ErrorCode=exports.ErrorCode || (exports.ErrorCode={}));
		var CustomFunctionError=function (_super) {
			function CustomFunctionError(errorCode, message) {
				var _this=_super.call(this, message) || this;
				return Object.setPrototypeOf(_this, CustomFunctionError.prototype), _this._code=errorCode || ErrorCode.invalidValue,
					_this._message=message || "", _this;
			}
			return __extends(CustomFunctionError, _super), Object.defineProperty(CustomFunctionError.prototype, "code", {
				get: function () {
					return this._code;
				},
				enumerable: !0,
				configurable: !0
			}), Object.defineProperty(CustomFunctionError.prototype, "message", {
				get: function () {
		...
		var CustomFunctionProxy=function () {
			...
			return CustomFunctionProxy.prototype._initSettings=function () {
				...
				try {
					...
					for (var entryArray=args.entries, invocationArray=[], cancellationArray=[], metadataArray=[], i=0; i < entryArray.length; i++)
						1===entryArray[i].messageCategory && ("string"==typeof entryArray[i].message && (entryArray[i].message=JSON.parse(entryArray[i].message)),
				...
				for (var i=0; i < entryArray.length; i++) {
					...
					if (_isNullOrUndefined(message))
						throw OfficeExtension.Utility.createRuntimeError(CustomFunctionRuntimeErrorCode.generalException, "message", "CustomFunctionProxy._handleMetadataEntries");
				...
				for (var i=0; i < entryArray.length; i++) {
					var message=entryArray[i].message;
					if (_isNullOrUndefined(message))
						throw OfficeExtension.Utility.createRuntimeError(CustomFunctionRuntimeErrorCode.generalException, "message", "CustomFunctionProxy._handleCancellationEntries");
				...
				for (var _this=this, batchArray=[], _loop_1=function (i) {
					...
					} : arrayOrObjectMessage, _isNullOrUndefined(message))
						throw OfficeExtension.Utility.createRuntimeError(CustomFunctionRuntimeErrorCode.generalException, "message", "CustomFunctionProxy._batchInvocationEntries");
					if (_isNullOrUndefined(message.invocationId) || message.invocationId < 0)
						throw OfficeExtension.Utility.createRuntimeError(CustomFunctionRuntimeErrorCode.generalException, "invocationId", "CustomFunctionProxy._batchInvocationEntries");
					if (_isNullOrUndefined(message.functionName))
						throw OfficeExtension.Utility.createRuntimeError(CustomFunctionRuntimeErrorCode.generalException, "functionName", "CustomFunctionProxy._batchInvocationEntries");
					var call=null, isCancelable=!1, isStreaming=!1;
					if ("number"==typeof message.flags)
						isCancelable=0 !=(1 & message.flags), isStreaming=0 !=(2 & message.flags);
					else {
					...
					if (this_1._invoker && !this_1._customFunctionMappingsContains(message.functionName))
						return this_1._invokeFunctionUsingInvoker(message),
							"continue";
					try {
					...
					if (isStreaming || isCancelable) {
						...
						invocationContext=new InvocationContext(message.functionName, message.address);
				...
				}, this_1=this, i=0; i < entryArray.length; i++)
					_loop_1(i);
				return batchArray;
				...
				if (_isNullOrUndefined(this._customFunctionMappingsUpperCase)) {
					if (this._customFunctionMappingsUpperCase={}, "object"==typeof CustomFunctionMappings)
						for (var key in OfficeExtension.CoreUtility.log("CustomFunctionMappings.Keys="+JSON.stringify(Object.keys(CustomFunctionMappings))),
							CustomFunctionMappings)
							this._customFunctionMappingsUpperCase[key.toUpperCase()] && CustomFunctionsLogger.logEvent(CustomFunctionProxy.CustomFunctionDuplicatedName, key),
								this._customFunctionMappingsUpperCase[key.toUpperCase()]=CustomFunctionMappings[key];
					if (this._association)
						for (var key in OfficeExtension.CoreUtility.log("CustomFunctionAssociateMappings.Keys="+JSON.stringify(Object.keys(this._association.mappings))),
							this._association.mappings)
							this._customFunctionMappingsUpperCase[key.toUpperCase()] && CustomFunctionsLogger.logEvent(CustomFunctionProxy.CustomFunctionDuplicatedName, key),
				...
				if ("undefined" !=typeof window) {
					for (var functionParent=window, functionNameSegments=functionName.split("."), i=0; i < functionNameSegments.length - 1; i++)
						if (functionParent=functionParent[functionNameSegments[i]],
							_isNullOrUndefined(functionParent) || "object" !=typeof functionParent)
							return !1;
					if ("function"==typeof functionParent[functionNameSegments[functionNameSegments.length - 1]])
						return !0;
				}
				return !1;
				...
				var functionNameUpperCase=functionName.toUpperCase();
				if (!_isNullOrUndefined(this._customFunctionMappingsUpperCase[functionNameUpperCase])) {
					if ("function"==typeof this._customFunctionMappingsUpperCase[functionNameUpperCase])
				...
				if (_isNullOrUndefined(window))
					throw OfficeExtension.Utility.createRuntimeError(CustomFunctionRuntimeErrorCode.invalidOperation, OfficeExtension.Utility._getResourceString(OfficeExtension.ResourceStrings.customFunctionWindowMissing), "CustomFunctionProxy._getFunction");
				for (var functionParent=window, functionNameSegments=functionName.split("."), i=0; i < functionNameSegments.length - 1; i++)
					if (functionParent=functionParent[functionNameSegments[i]],
						_isNullOrUndefined(functionParent) || "object" !=typeof functionParent)
						throw OfficeExtension.Utility.createRuntimeError(CustomFunctionRuntimeErrorCode.invalidOperation, OfficeExtension.Utility._getResourceString(OfficeExtension.ResourceStrings.customFunctionDefintionMissingOnWindow, functionName), "CustomFunctionProxy._getFunction");
				if ("function" !=typeof (call=functionParent[functionNameSegments[functionNameSegments.length - 1]]))
					throw OfficeExtension.Utility.createRuntimeError(CustomFunctionRuntimeErrorCode.invalidOperation, OfficeExtension.Utility._getResourceString(OfficeExtension.ResourceStrings.customFunctionInvalidFunction, functionName), "CustomFunctionProxy._getFunction");
				return call;
				...
				try {
					...
					for (var i=0; i < batch.invocationIds.length; i++)
						ex instanceof CustomFunctionError ? this._setError(batch.invocationIds[i], ex.message, this._getCustomFunctionResultErrorCodeFromErrorCode(ex.code)) : this._setError(batch.invocationIds[i], ex, 2);
				...
				if (batch.isStreaming)
					;
				else if (results.length===batch.parameterValueSets.length) {
					...
					for (i=0; i < batch.invocationIds.length; i++)
						this._setError(batch.invocationIds[i], OfficeExtension.Utility._getResourceString(OfficeExtension.ResourceStrings.customFunctionUnexpectedNumberOfEntriesInResultBatch), 4);
				}
				...
				if (this._setResultsTaskOverdueTime > 0 && timeNow > this._setResultsTaskOverdueTime)
					return OfficeExtension.Utility.log("SetResultsTask overdue"),
				...
				var timeNow=Date.now();
				if (this._inProgressContextSyncExpectedFinishTime > 0 && this._inProgressContextSyncExpectedFinishTime > timeNow)
					return OfficeExtension.Utility.log("context.sync() is in progress. setTimeout(CustomFunctionProxy._executeSetResultsTask)"),
						setTimeout(this._executeSetResultsTask.bind(this), this._setResultsDelayMillis),
				...
				for (var key in resultEntryBufferCopy)
					useArrayResult ? invocationResults.push(this._convertCustomFunctionInvocationResultToArray(resultEntryBufferCopy[key].result)) : invocationResults.push(resultEntryBufferCopy[key].result);
                        ...
                        try {
                            var parsedMessage = JSON.parse(args.message), error = parsedMessage.errorCode ? new OfficeExtension.Error(function(internalCode) {
                                ...
                                return table[internalCode] ? table[internalCode] : {
                                    code: "Unknown",
                                    message: "An unknown error has occured with code: " + internalCode
                                };
