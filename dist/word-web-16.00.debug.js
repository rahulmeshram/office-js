...
(function (OSFAppTelemetry) {
	...
	function initialize(context) {
		...
		appInfo.hostJSVersion="16.0.11330.30000";
...
(function (OSFWebAuth) {
	...
	function load(config) {
		console.log("webauth.implicit is being downloaded");
		OSF.OUtil.loadScript(OSF._OfficeAppFactory.getLoadScriptHelper().getOfficeJsBasePath()+CDN_PATH_WEBAUTHJS, function () {
			console.log("webauth.implicit was downloaded");
			if (config) {
				Implicit.Load(config);
				console.log("webauth.implicit was loaded");
			}
		});
	}
	OSFWebAuth.load=load;
	function getToken(target) {
		console.log("Implicit.GetToken is triggered");
...
(function (OfficeFirstPartyAuth) {
	var webauthconfig={};
	var IsWebAuthLoaded=false;
	function load(replyurl) {
		if (OSF.WebAuth && replyurl &&
			(typeof window.external==='undefined' || typeof window.external.GetContext==='undefined')) {
			webauthconfig={
				idp: 'aad',
				aadConfig: {
					...
					tenantId: '',
					upn: 'csstar9@pexprobe.ccsctp.net',
					timeout: 6000
				},
				enableConsoleLogging: true
			};
			OSF.WebAuth.load(webauthconfig);
			IsWebAuthLoaded=true;
			return true;
		}
		return false;
	}
		...
		if (OSF.WebAuth) {
			...
			return new OfficeExtension.CoreUtility.Promise(function (resolve, reject) {
				if (IsWebAuthLoaded) {
					...
					reject({ code: "NotInitialized", message: "Call OfficeFirstPartyAuth.load first before OfficeFirstPartyAuth.getAccessToken" });
				}
			});
		}
		else if (OSF._OfficeAppFactory.getHostInfo().hostPlatform=="web" && OSF._OfficeAppFactory.getHostInfo().hostType=="word") {
