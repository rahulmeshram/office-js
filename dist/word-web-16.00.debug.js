...
(function (OSFWebAuth) {
	...
	function getToken(target, popup) {
		...
		resultPromise.then(function () {
			logAcquireEvent(true, target, popup);
			return resultPromise;
		}, function (failure) {
			logAcquireEvent(false, target, popup, failure.ErrorCode);
			return resultPromise;
		});
	}
	...
	function logLoadEvent() {
		if (typeof OTel !=="undefined") {
			OTel.OTelLogger.onTelemetryLoaded(function () {
				...
				if (OSFWebAuth.config.telemetry) {
					for (var key in OSFWebAuth.config.telemetry) {
						telemetryData.push(oteljs.makeStringDataField(key, OSFWebAuth.config.telemetry[key]));
					}
				}
				...
				Office.context.webAuth.getAuthContextAsync(function (result) {
					if (result.status==="succeeded") {
						...
						OSF.WebAuth.config={
							...
							telemetry: { userId: authContext.userId }
						};
						OSF.WebAuth.load();
