			...
			try {
				Office.context.webAuth.getAuthContextAsync(function (result) {
					if (result.status==="succeeded") {
						...
						errorMessage=JSON.stringify(result.value);
					}
					OSF.WebAuth.load();
			...
			return new OfficeExtension.CoreUtility.Promise(function (resolve, reject) {
				if (!OSF.WebAuth.loaded) {
					if (!retrievedAuthContext) {
						reject({ code: ErrorCode.NoAuthContext, message: errorMessage });
					}
					reject({
						code: ErrorCode.NotLoaded,
						message: (Strings && Strings.OfficeOM.L_ImplicitNotLoaded) ? Strings.OfficeOM.L_ImplicitNotLoaded : ErrorCode.NotLoaded
					});
				}
