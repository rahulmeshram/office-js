...
(function (TrustResult) {
    TrustResult[TrustResult["Unknown"] = 0] = "Unknown";
    TrustResult[TrustResult["Trusted"] = 1] = "Trusted";
    TrustResult[TrustResult["Prompting"] = 2] = "Prompting";
...
var PathToStringFile = "../{locale}/office_strings.js";
var Strings = {};
...
function _loadStrings(locale, callback) {
    var localeOrDefault = OSF.getSupportedLocale(locale);
    var url = PathToStringFile.replace("{locale}", localeOrDefault);
    _loadJs(url, callback);
}
function _getConfirmedResultKey(hostname, noHttps) {
    return "OfficeJSTrustKey_" + (noHttps ? "http_" : "https_") + hostname;
}
function _hostTrustResult(hostname, noHttps) {
    var _localStorage = OSF.OUtil.getLocalStorage();
    var trustResult = JSON.parse(_localStorage.getItem(_getConfirmedResultKey(hostname, noHttps)));
    return (trustResult) ? trustResult.Result : TrustResult.Unknown;
}
function _confirmHost(hostname, noHttps, callback) {
    var keyResult = _getConfirmedResultKey(hostname, noHttps);
    var keyResultTimeStamp = keyResult + "TimeStamp";
    var hostTrustResult = _hostTrustResult(hostname, noHttps);
    if (hostTrustResult == TrustResult.Prompting) {
        var _localStorage = OSF.OUtil.getLocalStorage();
        var promptStarted = JSON.parse(_localStorage.getItem(keyResultTimeStamp));
        var elapseTimeMs = Date.now() - promptStarted;
        if (elapseTimeMs >= 30000) {
            hostTrustResult = TrustResult.Unknown;
        }
    }
    switch (+hostTrustResult) {
        case TrustResult.Unknown:
            function popUpConfirm() {
                var _localStorage = OSF.OUtil.getLocalStorage();
                var value = { Result: TrustResult.Prompting };
                _localStorage.setItem(keyResult, JSON.stringify(value));
                _localStorage.setItem(keyResultTimeStamp, JSON.stringify(Date.now()));
                var confirmMessage = Strings.OfficeOM["L_ConfirmDialog"].replace('{0}', hostname);
                if (noHttps) {
                    confirmMessage += " " + Strings.OfficeOM["L_NoHttpsWAC"];
                }
                var confirmed = confirm(confirmMessage);
                value.Result = (confirmed) ? TrustResult.Trusted : TrustResult.Unknown;
                _localStorage.setItem(keyResult, JSON.stringify(value));
                _localStorage.removeItem(keyResultTimeStamp);
                return confirmed;
            }
            callback(popUpConfirm());
            break;
        case TrustResult.Trusted:
            callback(true);
            break;
        case TrustResult.Prompting:
            function storageChanged(event) {
                OSF.OUtil.removeEventListener(window, "storage", storageChanged);
                var newTrustResult = _hostTrustResult(hostname, noHttps);
                callback(newTrustResult == TrustResult.Trusted);
            }
            OSF.OUtil.addEventListener(window, "storage", storageChanged);
            break;
    }
}
function _showInfoBar(hostname, noHttps, errorMessage, errorTitle) {
    ...
    if (noHttps) {
        var httpsMessageSpan = document.createElement("span");
        httpsMessageSpan.setAttribute("class", "moe-infobar-message");
        httpsMessageSpan.innerHTML = Strings.OfficeOM["L_NoHttpsWAC"];
        msgDiv.appendChild(document.createElement("br"));
        msgDiv.appendChild(document.createElement("br"));
        msgDiv.appendChild(httpsMessageSpan);
        msgDiv.appendChild(document.createElement("br"));
        msgDiv.appendChild(document.createElement("br"));
    }
    infoTableRows[0].cells[1].appendChild(msgDiv);
    var resultMessageSpan = document.createElement("span");
    resultMessageSpan.setAttribute("class", "moe-infobar-message");
    resultMessageSpan.setAttribute("style", "display:none");
    resultMessageSpan.innerHTML = Strings.OfficeOM["L_ConfirmCancelMessage"];
    var continueButton = document.createElement("button");
    continueButton.setAttribute("class", "moe-infobar-button");
    continueButton.setAttribute("type", "button");
    continueButton.innerHTML = Strings.OfficeOM["L_ContinueButton"];
    continueButton.onclick = function (event) {
        _confirmHost(hostname, noHttps, function (hostConfirmed) {
            if (hostConfirmed) {
                resultMessageSpan.innerHTML = Strings.OfficeOM["L_ConfirmRefreshMessage"];
            }
            continueButton.setAttribute("style", "display:none");
            resultMessageSpan.removeAttribute("style");
        });
    };
    infoTableRows[2].cells[1].appendChild(continueButton);
    infoTableRows[2].cells[1].appendChild(resultMessageSpan);
...
try {
    ...
    var e = Function._validateParams(urlParams, [{ name: "error", type: String, mayBeNull: false },
        { name: "locale", type: String, mayBeNull: false },
        { name: "hostname", type: String, mayBeNull: false },
        { name: "noHttps", type: Boolean, mayBeNull: false },
        { name: "validate", type: Boolean, mayBeNull: false }
    ]);
    if (!e) {
        if (urlParams.validate) {
            function validateHostnameTrust(event) {
                var receivedObj = JSON.parse(event.data);
                var e = Function._validateParams(receivedObj, [{ name: "hostname", type: String, mayBeNull: false },
                    { name: "noHttps", type: Boolean, mayBeNull: false }
                ]);
                if (!e) {
                    var hostTrustResult = _hostTrustResult(receivedObj.hostname, receivedObj.noHttps);
                    var hostUserTrusted = hostTrustResult === TrustResult.Trusted;
                    if (!hostUserTrusted) {
                        _loadStrings(urlParams.locale, function () {
                            _confirmHost(receivedObj.hostname, receivedObj.noHttps, function (hostConfirmed) {
                                var postingObj = { hostUserTrusted: hostConfirmed };
                                event.source.postMessage(JSON.stringify(postingObj), event.origin);
                            });
                        });
                    }
                    else {
                        var postingObj = { hostUserTrusted: hostUserTrusted };
                        event.source.postMessage(JSON.stringify(postingObj), event.origin);
                    }
                }
            }
            ;
            OSF.OUtil.addEventListener(window, "message", validateHostnameTrust);
        }
        else {
            _loadStrings(urlParams.locale, function () {
                if (SupportedError[urlParams.error])
                    _showInfoBar(urlParams.hostname, urlParams.noHttps, Strings.OfficeOM["L_" + urlParams.error]);
            });
        }
    }
}
