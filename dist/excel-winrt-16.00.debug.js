...
OSF.InitializationHelper.prototype.getAppContext=function OSF_InitializationHelper$getAppContext(wnd, gotAppContext) {
	var getInvocationCallback=function OSF_InitializationHelper_getAppContextAsync$getInvocationCallbackWebApp(appContext) {
		...
		returnedContext=new OSF.OfficeAppContext(id, appType, version, UILocale, dataLocale, docUrl, clientMode, serializedSettings, reason, osfControlType, eToken, correlationId, appInstanceId, touchEnabled, commerceAllowed, minorVersion, requirementMatrix, undefined, hostFullVersion, undefined, undefined, undefined, undefined, undefined, undefined, undefined, officeThemeInfo);
		if (OSF.AppTelemetry) {
...
var _BeginOfficeRuntime="_BeginOfficeRuntime";
var OfficeExtension;
...
++scalarPropCount;
	...
	var LibraryBuilder=(function () {
		function LibraryBuilder(options) {
			...
			this.m_nsObj=options.targetNamespaceObject;
			...
			this.m_typeNames=metadata.typeNames;
				...
				for (var i=0; i < metadata.clientObjectTypes.length; i++) {
					...
					if (Array.isArray(elem)) {
						metadata.clientObjectTypes[i]={
							...
							collectionPropertyPath: this.getStringFromCompactJsonValue(elem[2]),
							newObjectServerTypeFullName: this.getStringFromCompactJsonValue(elem[3]),
							newObjectApiFlags: elem[4],
							childItemTypeFullName: this.getTypeNameFromTypeNameOrdinalOrName(elem[5]),
							scalarProperties: elem[6],
							navigationProperties: elem[7],
							scalarMethods: elem[8],
							navigationMethods: elem[9],
							events: elem[10],
							v0Events: elem[11]
						};
					}
					this.buildClientObjectType(metadata.clientObjectTypes[i], options.noDelayInitialize);
				}
			}
		}
		LibraryBuilder.prototype.getStringFromCompactJsonValue=function (value) {
			if (value===0) {
				return null;
			}
			return value;
		};
		LibraryBuilder.prototype.getTypeNameFromTypeNameOrdinalOrName=function (value) {
			if (typeof (value)==="number") {
				if (value > 0) {
					return this.m_typeNames[value - 1];
				}
				return null;
			}
			return value;
		...
		LibraryBuilder.prototype.buildEnumType=function (elem) {
			...
			this.m_nsObj[enumType.name]=enumType.fields;
		};
		LibraryBuilder.prototype.buildClientObjectType=function (typeInfo, noDelayInitialize) {
			var thisBuilder=this;
			var type=function () {
				OfficeExtension.ClientObject.apply(this, arguments);
				if (!thisBuilder.m_nsObj[typeInfo.name]._typeInited) {
					thisBuilder.buildPrototype(thisBuilder.m_nsObj[typeInfo.name], typeInfo);
					thisBuilder.m_nsObj[typeInfo.name]._typeInited=true;
				}
			};
			this.m_nsObj[typeInfo.name]=type;
			this.extendsType(type, OfficeExtension.ClientObject);
		...
		LibraryBuilder.prototype.findObjByPath=function (top, parts, start) {
			var obj=top;
			for (var i=start; i < parts.length; i++) {
				if (typeof (obj) !=='object') {
					throw new OfficeExtension_1.Error("Cannot find "+parts.join("."));
				}
				obj=obj[parts[i]];
			}
			return obj;
		};
		LibraryBuilder.prototype.getTypeOrFunction=function (fullName) {
			var ret=this.resolveObjectByName(fullName);
			if (typeof (ret) !=='function') {
				throw new OfficeExtension_1.Error("Cannot find type: "+fullName);
			}
			return ret;
		};
		LibraryBuilder.prototype.resolveObjectByName=function (fullName) {
			var parts=fullName.split('.');
			if (parts.length===1) {
				return this.m_nsObj[parts[0]];
			}
			if (parts[0]===this.m_namespace) {
				return this.findObjByPath(this.m_nsObj, parts, 1);
			}
			if (this.m_namespaceMap[parts[0]]) {
				return this.findObjByPath(this.m_namespaceMap[parts[0]], parts, 1);
			}
			return this.findObjByPath(this.m_nsObj, parts, 0);
		};
		...
		LibraryBuilder.prototype.evaluateEventType=function (eventTypeExpression) {
			...
			for (var i=0; i < eventTypeExpression.length; i++) {
				if (eventTypeExpression.charCodeAt(i) < charZero ||
					eventTypeExpression.charCodeAt(i) > charNine) {
					allDigit=false;
					break;
				}
			}
			...
			var ret=this.resolveObjectByName(eventTypeExpression);
				...
				for (var i=0; i < typeInfo.scalarProperties.length; i++) {
					...
					if (Array.isArray(elem)) {
						typeInfo.scalarProperties[i]={
							...
							originalName: this.getStringFromCompactJsonValue(elem[3]),
				...
				for (var i=0; i < typeInfo.navigationProperties.length; i++) {
					...
					if (Array.isArray(elem)) {
						typeInfo.navigationProperties[i]={
							...
							apiSetInfoOrdinal: elem[2],
							originalName: this.getStringFromCompactJsonValue(elem[3]),
							getMethodApiFlags: elem[4],
							setMethodApiFlags: elem[5],
							propertyTypeFullName: this.getTypeNameFromTypeNameOrdinalOrName(elem[6])
						};
					}
					this.buildNavigationProperty(type, typeInfo, typeInfo.navigationProperties[i]);
				}
			}
			if (typeInfo.scalarMethods) {
				...
				for (var i=0; i < typeInfo.navigationMethods.length; i++) {
					...
					if (Array.isArray(elem)) {
						typeInfo.navigationMethods[i]={
							...
							apiFlags: elem[4],
							parameterCount: elem[5],
							returnTypeFullName: this.getTypeNameFromTypeNameOrdinalOrName(elem[6]),
							returnObjectGetByIdMethodName: this.getStringFromCompactJsonValue(elem[7])
						};
					}
				...
				for (var i=0; i < typeInfo.events.length; i++) {
					...
					if (Array.isArray(elem)) {
						typeInfo.events[i]={
							...
							behaviorFlags: elem[1],
							apiSetInfoOrdinal: elem[2],
							typeExpression: elem[3],
							targetIdExpression: elem[4],
							registerMethodName: elem[5],
							unregisterMethodName: elem[6]
						};
					}
					this.buildEvent(type, typeInfo, typeInfo.events[i]);
				}
			}
			if (typeInfo.v0Events) {
				for (var i=0; i < typeInfo.v0Events.length; i++) {
					var elem=typeInfo.v0Events[i];
					if (Array.isArray(elem)) {
						typeInfo.v0Events[i]={
							name: elem[0],
							behaviorFlags: elem[1],
							...
							typeExpression: elem[3],
							targetIdExpression: elem[4],
							registerFuncName: elem[5],
							unregisterFuncName: elem[6]
						};
					}
					this.buildV0Event(type, typeInfo, typeInfo.v0Events[i]);
				}
			}
		...
		LibraryBuilder.prototype.throwIfApiNotSupported=function (typeInfo, member) {
			if (this.m_apiSets && member.apiSetInfoOrdinal > 0) {
				...
				if (apiSetInfo) {
					OfficeExtension.Utility.throwIfApiNotSupported(typeInfo.name+"."+member.name, apiSetInfo.name, apiSetInfo.version, this.m_hostName);
				}
			}
		};
		LibraryBuilder.prototype.buildScalarProperty=function (type, typeInfo, propInfo) {
			...
			var descriptor={
				get: function () {
					OfficeExtension.Utility.throwIfNotLoaded(propInfo.name, this[fieldName], typeInfo.name, this._isNull);
			...
			if ((propInfo.behaviorFlags & 2)===0) {
				descriptor.set=function (value) {
					if (propInfo.behaviorFlags & 4) {
						var customizationFunc=thisBuilder.getTypeOrFunction("_CC."+typeInfo.name+"_"+propInfo.originalName+"_Set");
					...
					OfficeExtension.BatchApiHelper.invokeSetProperty(this, propInfo.originalName, value, propInfo.setMethodApiFlags);
				};
			}
			...
			var descriptor={
				get: function () {
					if (!this[thisBuilder.getFieldName(propInfo)]) {
						thisBuilder.throwIfApiNotSupported(typeInfo, propInfo);
						this[fieldName]=OfficeExtension.BatchApiHelper.createPropertyObject(thisBuilder.getTypeOrFunction(propInfo.propertyTypeFullName), this, propInfo.originalName, (propInfo.behaviorFlags & 16) !==0, propInfo.getMethodApiFlags);
					}
					if (propInfo.behaviorFlags & 64) {
						var customizationFunc=thisBuilder.getTypeOrFunction("_CC."+typeInfo.name+"_"+propInfo.originalName+"_Get");
		...
		LibraryBuilder.prototype.buildScalarMethod=function (type, typeInfo, methodInfo) {
			...
			type.prototype[methodInfo.name]=function () {
				...
				if (methodInfo.behaviorFlags & 1) {
					var customizationFunc=thisBuilder.getTypeOrFunction("_CC."+typeInfo.name+"_"+methodInfo.originalName);
					var applyArgs=[this];
					for (var i=0; i < args.length; i++) {
				...
				return OfficeExtension.BatchApiHelper.invokeMethod(this, methodInfo.originalName, operationType, args, methodInfo.apiFlags, resultProcessType);
			};
		};
		LibraryBuilder.prototype.buildNavigationMethod=function (type, typeInfo, methodInfo) {
			...
			type.prototype[methodInfo.name]=function () {
				...
				if ((methodInfo.behaviorFlags & 16) !==0) {
					return OfficeExtension.BatchApiHelper.createIndexerObject(thisBuilder.getTypeOrFunction(methodInfo.returnTypeFullName), this, args);
				}
				else {
					var operationType=0;
					if (methodInfo.behaviorFlags & 2) {
					...
					return OfficeExtension.BatchApiHelper.createMethodObject(thisBuilder.getTypeOrFunction(methodInfo.returnTypeFullName), this, methodInfo.originalName, operationType, args, (methodInfo.behaviorFlags & 4) !==0, (methodInfo.behaviorFlags & 8) !==0, methodInfo.returnObjectGetByIdMethodName, methodInfo.apiFlags);
				}
			};
		};
		LibraryBuilder.prototype.buildHandleResult=function (type, typeInfo) {
			...
			type.prototype._handleResult=function (value) {
				OfficeExtension.ClientObject.prototype._handleResult.call(this, value);
				if (Utility.isNullOrUndefined(value)) {
					return;
				}
				OfficeExtension.Utility.fixObjectPathIfNecessary(this, value);
				if (typeInfo.behaviorFlags & 8) {
					var customizationFunc=thisBuilder.getTypeOrFunction("_CC."+typeInfo.name+"_HandleResult");
				...
				if (typeInfo.navigationProperties) {
					...
					OfficeExtension.Utility._handleNavigationPropertyResults(this, value, propNames);
				}
				if ((typeInfo.behaviorFlags & 1) !==0) {
					var hasIndexerMethod=thisBuilder.hasIndexMethod(typeInfo);
					if (!Utility.isNullOrUndefined(value[OfficeExtension.Constants.items])) {
						this.m__items=[];
						var _data=value[OfficeExtension.Constants.items];
						var childItemType=thisBuilder.getTypeOrFunction(typeInfo.childItemTypeFullName);
		...
		LibraryBuilder.prototype.buildHandleRetrieveResult=function (type, typeInfo) {
			...
			type.prototype._handleRetrieveResult=function (value, result) {
				OfficeExtension.ClientObject.prototype._handleRetrieveResult.call(this, value, result);
				if (Utility.isNullOrUndefined(value)) {
					return;
				}
				if (typeInfo.scalarProperties) {
				...
				if (typeInfo.behaviorFlags & 1) {
					...
					var childItemType_1=thisBuilder.getTypeOrFunction(typeInfo.childItemTypeFullName);
					var thisObj_1=this;
					...
					OfficeExtension.Utility.processRetrieveResult(this, value, result);
				}
			};
		};
		LibraryBuilder.prototype.buildHandleIdResult=function (type, typeInfo) {
			...
			type.prototype._handleIdResult=function (value) {
				OfficeExtension.ClientObject.prototype._handleIdResult.call(this, value);
				if (OfficeExtension.Utility.isNullOrUndefined(value)) {
					return;
				}
				if (typeInfo.behaviorFlags & 16) {
					var customizationFunc=thisBuilder.getTypeOrFunction("_CC."+typeInfo.name+"_HandleIdResult");
					customizationFunc.call(this, this, value);
				}
				if (typeInfo.scalarProperties) {
					for (var i=0; i < typeInfo.scalarProperties.length; i++) {
		...
		LibraryBuilder.prototype.buildNewObject=function (type, typeInfo) {
			if (!Utility.isNullOrEmptyString(typeInfo.newObjectServerTypeFullName)) {
				type.newObject=function (context) {
					return OfficeExtension.BatchApiHelper.createTopLevelServiceObject(type, context, typeInfo.newObjectServerTypeFullName, (typeInfo.behaviorFlags & 1) !==0, typeInfo.newObjectApiFlags);
				};
			}
		};
		LibraryBuilder.prototype.buildSetMockData=function (type, typeInfo) {
			...
			if (typeInfo.behaviorFlags & 1) {
				...
				type.prototype.setMockData=function (data) {
					var thisObj=this;
					OfficeExtension.Utility.setMockData(thisObj, data, function (childItemData, index) {
						return BatchApiHelper.createChildItemObject(thisBuilder.getTypeOrFunction(typeInfo.childItemTypeFullName), hasIndexMethod_1, thisObj, childItemData, index);
					}, function (items) {
		...
		LibraryBuilder.prototype.buildItems=function (type, typeInfo) {
			...
			Object.defineProperty(type.prototype, "items", {
				get: function () {
					OfficeExtension.Utility.throwIfNotLoaded("items", this.m__items, typeInfo.name, this._isNull);
		...
		LibraryBuilder.prototype.buildToJSON=function (type, typeInfo) {
			...
			if ((typeInfo.behaviorFlags & 1) !==0) {
				type.prototype.toJSON=function () {
					return OfficeExtension.Utility.toJson(this, {}, {}, this.m__items);
				};
				return;
			}
			else {
				type.prototype.toJSON=function () {
					...
					return OfficeExtension.Utility.toJson(this, scalarProperties, navProperties);
				};
			}
		};
		...
		LibraryBuilder.prototype.buildMixin=function (type, typeInfo) {
			if (typeInfo.behaviorFlags & 4) {
				var mixinType=this.getTypeOrFunction(typeInfo.name+'Custom');
			...
			Object.defineProperty(type.prototype, eventName, {
				get: function () {
					if (!this[fieldName]) {
						...
						if (evt.registerMethodName !=='null') {
							registerFunc=this[evt.registerMethodName].bind(this);
						}
						var unregisterFunc=null;
						if (evt.unregisterMethodName !=='null') {
							unregisterFunc=this[evt.unregisterMethodName].bind(this);
						}
						...
						if (evt.behaviorFlags & 1) {
							func=thisBuilder.getTypeOrFunction("_CC."+typeInfo.name+"_"+evt.name+"_EventArgsTransform");
						}
						var eventArgsTransformFunc=function (value) {
							if (func) {
								value=func.call(thisObj, thisObj, value);
							}
							return OfficeExtension.Utility._createPromiseFromResult(value);
						};
						var eventType=thisBuilder.evaluateEventType(evt.typeExpression);
						this[fieldName]=new OfficeExtension.GenericEventHandlers(this.context, this, evt.name, {
							eventType: eventType,
			...
			Object.defineProperty(type.prototype, eventName, {
				get: function () {
					if (!this[fieldName]) {
						...
						var thisObj=this;
						var registerFunc=null;
						if (Utility.isNullOrEmptyString(evt.registerFuncName)) {
						...
						else if (evt.registerFuncName !=='null') {
							var func_1=thisBuilder.getTypeOrFunction(evt.registerFuncName);
							registerFunc=								function (handlerCallback) {
									return func_1.call(thisObj, thisObj, handlerCallback);
								};
						}
						var unregisterFunc=null;
						if (Utility.isNullOrEmptyString(evt.unregisterFuncName)) {
						...
						else if (evt.unregisterFuncName !=='null') {
							var func_2=thisBuilder.getTypeOrFunction(evt.unregisterFuncName);
							unregisterFunc=								function (handlerCallback) {
						...
						this[fieldName]=new OfficeExtension.EventHandlers(this.context, this, evt.name, {
							registerFunc: registerFunc,
							unregisterFunc: unregisterFunc,
							eventArgsTransformFunc: eventArgsTransformFunc
						});
...
var _EndOfficeRuntime="_EndOfficeRuntime";
var __extends=(this && this.__extends) || (function () {
...
(function (OfficeCore) {
	...
	var DynamicRibbon=(function (_super) {
		...
		DynamicRibbon.prototype.executeRequestUpdate=function (jsonUpdate) {
			return _createMethodObject(OfficeCore.RibbonButton, this, "ExecuteRequestUpdate", 0, [jsonUpdate], false, false, null, 0);
		};
		DynamicRibbon.prototype.getButton=function (id) {
			return _createMethodObject(OfficeCore.RibbonButton, this, "GetButton", 0, [id], false, false, null, 0);
		};
		DynamicRibbon.prototype.getTab=function (id) {
			return _createMethodObject(OfficeCore.RibbonTab, this, "GetTab", 0, [id], false, false, null, 0);
		};
	...
	var _typeOfficeServicesManagerApi="OfficeServicesManagerApi";
	var OfficeServicesManagerApi=(function (_super) {
		...
		OfficeServicesManagerApi.prototype.bindServiceToProfile=function (servicePrefix, identityUniqueId) {
			return _invokeMethod(this, "BindServiceToProfile", 0, [servicePrefix, identityUniqueId], 0, 0);
		};
		OfficeServicesManagerApi.prototype._handleResult=function (value) {
			_super.prototype._handleResult.call(this, value);
			if (_isNullOrUndefined(value))
				return;
			var obj=value;
		...
		OfficeServicesManagerApi.prototype._handleRetrieveResult=function (value, result) {
			_super.prototype._handleRetrieveResult.call(this, value, result);
			_processRetrieveResult(this, value, result);
		};
		OfficeServicesManagerApi.newObject=function (context) {
			return _createTopLevelServiceObject(OfficeCore.OfficeServicesManagerApi, context, "Microsoft.OfficeServicesManager.OfficeServicesManagerApi", false, 4);
		};
	...
	OfficeCore.OfficeServicesManagerApi=OfficeServicesManagerApi;
	...
	(function (ErrorCodes) {
		...
		ErrorCodes["ribbonApiNotAvailable"]="RibbonApiNotAvailable";
...
(function (Excel) {
	...
	var _libraryMetadata={ "version": "1.0.0",
		"name": "Excel",
		"defaultApiSetName": "ExcelApi",
		"hostName": "Excel",
		"apiSets": [["1.8"], ["1.9"], ["1.11"], ["1.6"], ["1.7"], ["1.12"], ["1.2"], ["1.3"], ["1.4"], ["1.5"], ["1.10"], ["1.1", "CustomFunctions"], ["1.1", "WACOperationEvents"], ["1.1", "ExcelApiOnline"], ["99.9"]],
		"typeNames": ["IterativeCalculation", "Ribbon", "CultureInfo", "WorkbookCreated", "WorksheetCollection", "NamedItemCollection", "TableCollection", "Application", "BindingCollection", "Functions", "_V1Api", "PivotTableCollection", "SettingCollection", "CustomXmlPartCollection", "InternalTest", "DocumentProperties", "StyleCollection", "WorkbookProtection", "DataConnectionCollection", "Runtime", "CommentCollection", "SlicerCollection", "TableStyleCollection", "PivotTableStyleCollection", "SlicerStyleCollection", "TimelineStyleCollection", "Range", "Chart", "RangeAreas", "Slicer", "ChartCollection", "WorksheetProtection", "WorksheetFreezePanes", "PageLayout", "VisualCollection", "ShapeCollection", "PageBreakCollection", "AutoFilter", "WorksheetCustomPropertyCollection", "Worksheet", "RangeFormat", "RangeSort", "ConditionalFormatCollection", "DataValidation", "RangeView", "TableScopedCollection", "RemoveDuplicatesResult", "RangeCollection", "RangeViewCollection", "Setting", "NamedItem", "NamedItemArrayValues", "Table", "Binding", "TableColumnCollection", "TableRowCollection", "TableSort", "TableStyle", "TableColumn", "Filter", "TableRow", "RangeFill", "RangeFont", "RangeBorderCollection", "FormatProtection", "RangeBorder", "ChartTitle", "ChartDataLabels", "ChartLegend", "ChartSeriesCollection", "ChartAxes", "ChartAreaFormat", "ChartPlotArea", "ChartPivotOptions", "ChartFill", "ChartFont", "ChartBorder", "ChartSeries", "ChartPointsCollection", "ChartSeriesFormat", "ChartTrendlineCollection", "ChartErrorBars", "ChartBinOptions", "ChartMapOptions", "ChartBoxwhiskerOptions", "ChartLineFormat", "ChartPoint", "ChartPointFormat", "ChartDataLabel", "ChartAxis", "ChartGridlines", "ChartAxisTitle", "ChartAxisFormat", "ChartAxisTitleFormat", "ChartDataLabelFormat", "ChartErrorBarsFormat", "ChartGridlinesFormat", "ChartLegendFormat", "ChartLegendEntryCollection", "ChartLegendEntry", "ChartTitleFormat", "ChartFormatString", "ChartTrendlineFormat", "ChartTrendlineLabel", "ChartTrendline", "ChartTrendlineLabelFormat", "ChartPlotAreaFormat", "Visual", "VisualPropertyCollection", "DataControllerClient", "VisualProperty", "NumberFormatInfo", "CustomXmlPart", "CustomXmlPartScopedCollection", "PivotTable", "PivotHierarchyCollection", "RowColumnPivotHierarchyCollection", "DataPivotHierarchyCollection", "FilterPivotHierarchyCollection", "PivotLayout", "PivotTableStyle", "DataPivotHierarchy", "PivotHierarchy", "PivotFieldCollection", "RowColumnPivotHierarchy", "FilterPivotHierarchy", "PivotField", "PivotItemCollection", "PivotItem", "WorksheetCustomProperty", "CustomPropertyCollection", "CustomProperty", "ConditionalFormat", "DataBarConditionalFormat", "CustomConditionalFormat", "IconSetConditionalFormat", "ColorScaleConditionalFormat", "TopBottomConditionalFormat", "PresetCriteriaConditionalFormat", "TextConditionalFormat", "CellValueConditionalFormat", "ConditionalDataBarPositiveFormat", "ConditionalDataBarNegativeFormat", "ConditionalFormatRule", "ConditionalRangeFormat", "ConditionalRangeFill", "ConditionalRangeFont", "ConditionalRangeBorderCollection", "ConditionalRangeBorder", "NumberFormatter", "Style", "SlicerStyle", "TimelineStyle", "HeaderFooterGroup", "HeaderFooter", "PageBreak", "Comment", "CommentReplyCollection", "CommentReply", "Shape", "GeometricShape", "Image", "TextFrame", "ShapeFill", "ShapeGroup", "Line", "ShapeLineFormat", "GroupShapeCollection", "TextRange", "ShapeFont", "SlicerItemCollection", "SlicerItem", "FunctionResult"],
		"enumTypes": [["BuiltInPivotTableStyle", ["none", "light1", "light2", "light3", "light4", "light5", "light6", "light7", "light8", "light9", "light10", "light11", "light12", "light13", "light14", "light15", "light16", "light17", "light18", "light19", "light20", "light21", "light22", "light23", "light24", "light25", "light26", "light27", "light28", "medium1", "medium2", "medium3", "medium4", "medium5", "medium6", "medium7", "medium8", "medium9", "medium10", "medium11", "medium12", "medium13", "medium14", "medium15", "medium16", "medium17", "medium18", "medium19", "medium20", "medium21", "medium22", "medium23", "medium24", "medium25", "medium26", "medium27", "medium28", "dark1", "dark2", "dark3", "dark4", "dark5", "dark6", "dark7", "dark8", "dark9", "dark10", "dark11", "dark12", "dark13", "dark14", "dark15", "dark16", "dark17", "dark18", "dark19", "dark20", "dark21", "dark22", "dark23", "dark24", "dark25", "dark26", "dark27", "dark28"]],
			...
			["MessageCategory", [], { "none": 0, "customFunction": 1, "event": 65536 }],
		...
		"clientObjectTypes": [["Runtime", 0, 0, 0, 0, 0, [["enableEvents", 1, 1, 0, 0]]],
			["Application", 0, 0, 0, 0, 0, [["calculationMode", 1, 0, 0, 0], ["calculationEngineVersion", 3, 2, 0, 0], ["calculationState", 3, 2, 0, 0], ["decimalSeparator", 3, 3, 0, 0], ["thousandsSeparator", 3, 3, 0, 0], ["useSystemSeparators", 3, 3, 0, 0]], [["iterativeCalculation", 35, 2, 0, 4, 0, 1], ["ribbon", 35, 2, 0, 4, 0, 2], ["cultureInfo", 3, 3, 0, 4, 0, 3]], [["calculate", 0, 0, 0, 0, 1], ["suspendApiCalculationUntilNextSync", 1, 4, 0, 0, 0], ["suspendScreenUpdatingUntilNextSync", 0, 2, 0, 0, 0]], [["createWorkbook", 10, 1, 0, 0, 1, 4, "_GetWorkbookCreatedById"], ["_GetWorkbookCreatedById", 2, 1, 0, 4, 1, 4, 0]]],
			["IterativeCalculation", 0, 0, 0, 0, 0, [["enabled", 1, 0, 0, 0], ["maxIteration", 1, 0, 0, 0], ["maxChange", 1, 0, 0, 0]]],
			["Workbook", 0, 0, 0, 0, 0, [["name", 3, 5, 0, 0], ["readOnly", 3, 1, 0, 0], ["isDirty", 1, 2, 0, 0], ["use1904DateSystem", 1, 6, 0, 0], ["chartDataPointTrack", 1, 2, 0, 0], ["usePrecisionAsDisplayed", 1, 2, 0, 0], ["calculationEngineVersion", 3, 2, 0, 0], ["autoSave", 3, 2, 0, 0], ["previouslySaved", 3, 2, 0, 0]], [["worksheets", 19, 0, 0, 4, 0, 5], ["names", 19, 0, 0, 4, 0, 6], ["tables", 19, 0, 0, 4, 0, 7], ["application", 2, 0, 0, 4, 0, 8], ["bindings", 19, 0, 0, 4, 0, 9], ["functions", 2, 7, 0, 4, 0, 10], ["_V1Api", 2, 8, 0, 4, 0, 11], ["pivotTables", 19, 8, 0, 4, 0, 12], ["settings", 19, 9, 0, 4, 0, 13], ["customXmlParts", 19, 10, 0, 4, 0, 14], ["internalTest", 2, 4, 0, 4, 0, 15], ["properties", 35, 5, 0, 4, 0, 16], ["styles", 19, 5, 0, 4, 0, 17], ["protection", 3, 5, 0, 4, 0, 18], ["dataConnections", 3, 5, 0, 4, 0, 19], ["_Runtime", 2, 10, 0, 4, 0, 20], ["comments", 19, 11, 0, 4, 0, 21], ["slicers", 19, 11, 0, 4, 0, 22], ["tableStyles", 19, 11, 0, 4, 0, 23], ["pivotTableStyles", 19, 11, 0, 4, 0, 24], ["slicerStyles", 19, 11, 0, 4, 0, 25], ["timelineStyles", 19, 11, 0, 4, 0, 26]], [["_RemoveReference", 2, 0, 0, 0, 1], ["_GetObjectByReferenceId", 2, 0, 0, 4, 1], ["_GetObjectTypeNameByReferenceId", 2, 0, 0, 4, 1], ["_RemoveAllReferences", 2, 0, 0, 0, 0], ["_GetReferenceCount", 2, 0, 0, 4, 0], ["getIsActiveCollabSession", 0, 2, 0, 0, 0], ["registerCustomFunctions", 0, 12, 0, 8, 7], ["_SetOsfControlContainerReadyForCustomFunctions", 0, 12, 0, 0, 0], ["close", 0, 3, 0, 0, 1], ["save", 0, 3, 0, 0, 1], ["_RegisterAutoSaveSettingChangedEvent", 0, 2, 0, 0, 0], ["_UnregisterAutoSaveSettingChangedEvent", 0, 2, 0, 0, 0], ["_RegisterWACOperationEvent", 0, 13, 0, 0, 0], ["_UnregisterWACOperationEvent", 0, 13, 0, 0, 0]], [["getSelectedRange", 10, 0, 0, 4, 0, 27, 0], ["getActiveCell", 10, 5, 0, 4, 0, 27, 0], ["_GetRangeForEventByReferenceId", 2, 0, 0, 4, 1, 27, 0], ["_GetRangeOrNullObjectForEventByReferenceId", 2, 0, 0, 4, 1, 27, 0], ["getActiveChart", 2, 2, 0, 4, 0, 28, 0], ["getActiveChartOrNullObject", 2, 2, 0, 4, 0, 28, 0], ["getSelectedRanges", 10, 2, 0, 4, 0, 29, 0], ["_GetRangesForEventByReferenceId", 2, 2, 0, 4, 1, 29, 0], ["_GetRangesOrNullObjectForEventByReferenceId", 2, 2, 0, 4, 1, 29, 0], ["getActiveSlicer", 2, 11, 0, 4, 0, 30, 0], ["getActiveSlicerOrNullObject", 2, 11, 0, 4, 0, 30, 0]], [["AutoSaveSettingChanged", 0, 2, "MessageType.workbookAutoSaveSettingChangedEvent", "OfficeExtension.Constants.eventWorkbookId", "_RegisterAutoSaveSettingChangedEvent", "_UnregisterAutoSaveSettingChangedEvent"], ["WACOperationEvent", 1, 13, "MessageType.wacoperationEvent", "OfficeExtension.Constants.eventWorkbookId", "_RegisterWACOperationEvent", "_UnregisterWACOperationEvent"]], [["SelectionChanged", 1, 8, "_CC.office10EventIdDocumentSelectionChangedEvent", "", "", ""], ["_Message", 1, 5, "_CC.office10EventIdRichApiMessageEvent", "", "", ""]]],
			["WorkbookProtection", 0, 0, 0, 0, 0, [["protected", 3, 0, 0, 0]], [], [["protect", 0, 0, 0, 0, 1], ["unprotect", 0, 0, 0, 0, 1]]],
			["WorkbookCreated", 0, 0, 0, 0, 0, [["id", 3, 0, 0, 0]], [], [["open", 2, 0, 0, 4, 0]]],
			["Worksheet", 0, "workbook.worksheets", 0, 0, 0, [["name", 1, 0, 0, 0], ["id", 3, 0, 0, 0], ["position", 1, 0, 0, 0], ["visibility", 1, 0, 0, 0], ["tabColor", 1, 5, 0, 0], ["standardWidth", 1, 5, 0, 0], ["standardHeight", 3, 5, 0, 0], ["showGridlines", 5, 1, 0, 0], ["showHeadings", 5, 1, 0, 0], ["enableCalculation", 1, 2, 0, 0]], [["charts", 83, 0, 0, 4, 0, 31], ["tables", 83, 0, 0, 4, 0, 7], ["protection", 3, 7, 0, 4, 0, 32], ["pivotTables", 19, 8, 0, 4, 0, 12], ["names", 19, 9, 0, 4, 0, 6], ["freezePanes", 3, 5, 0, 4, 0, 33], ["pageLayout", 35, 2, 0, 4, 0, 34], ["visuals", 83, 11, 0, 4, 0, 35], ["shapes", 19, 2, 0, 4, 0, 36], ["horizontalPageBreaks", 19, 2, 0, 4, 0, 37], ["verticalPageBreaks", 19, 2, 0, 4, 0, 37], ["autoFilter", 3, 2, 0, 4, 0, 38], ["slicers", 19, 11, 0, 4, 0, 22], ["comments", 19, 11, 0, 4, 0, 21], ["customProperties", 19, 6, 0, 4, 0, 39]], [["delete", 0, 0, 0, 0, 0], ["activate", 2, 0, 0, 0, 0], ["calculate", 0, 4, 0, 0, 1], ["_RegisterDataChangedEvent", 0, 5, 0, 0, 0], ["_UnregisterDataChangedEvent", 0, 5, 0, 0, 0], ["_RegisterActivatedEvent", 0, 5, 0, 0, 0], ["_UnregisterActivatedEvent", 0, 5, 0, 0, 0], ["_RegisterDeactivatedEvent", 0, 5, 0, 0, 0], ["_UnregisterDeactivatedEvent", 0, 5, 0, 0, 0], ["_RegisterSelectionChangedEvent", 0, 5, 0, 0, 0], ["_UnregisterSelectionChangedEvent", 0, 5, 0, 0, 0], ["_RegisterCalculatedEvent", 0, 1, 0, 0, 0], ["_UnregisterCalculatedEvent", 0, 1, 0, 0, 0], ["replaceAll", 0, 2, 0, 0, 3], ["_RegisterFilteredEvent", 0, 6, 0, 0, 0], ["_UnregisterFilteredEvent", 0, 6, 0, 0, 0], ["_RegisterFormatChangedEvent", 0, 2, 0, 0, 0], ["_UnregisterFormatChangedEvent", 0, 2, 0, 0, 0], ["_RegisterRowSortedEvent", 0, 11, 0, 0, 0], ["_UnregisterRowSortedEvent", 0, 11, 0, 0, 0], ["_RegisterColumnSortedEvent", 0, 11, 0, 0, 0], ["_UnregisterColumnSortedEvent", 0, 11, 0, 0, 0], ["_RegisterSingleClickedEvent", 0, 11, 0, 0, 0], ["_UnregisterSingleClickedEvent", 0, 11, 0, 0, 0], ["_RegisterRowHiddenChangedEvent", 0, 3, 0, 0, 0], ["_UnregisterRowHiddenChangedEvent", 0, 3, 0, 0, 0], ["showOutlineLevels", 0, 11, 0, 0, 2]], [["getRange", 10, 0, 0, 4, 1, 27, 0], ["getUsedRange", 10, 0, 0, 4, 1, 27, 0], ["getCell", 10, 0, 0, 4, 2, 27, 0], ["getUsedRangeOrNullObject", 10, 9, 0, 4, 1, 27, 0], ["getRangeByIndexes", 10, 5, 0, 4, 4, 27, 0], ["getPrevious", 10, 10, 0, 4, 1, 40, "_GetSheetById"], ["getPreviousOrNullObject", 10, 10, 0, 4, 1, 40, "_GetSheetById"], ["getNext", 10, 10, 0, 4, 1, 40, "_GetSheetById"], ["getNextOrNullObject", 10, 10, 0, 4, 1, 40, "_GetSheetById"], ["_GetSheetById", 2, 5, 0, 4, 1, 40, 0], ["copy", 0, 5, 0, 0, 2, 40, "_GetAnotherWorksheetById"], ["_GetAnotherWorksheetById", 0, 5, 0, 0, 1, 40, 0], ["findAll", 10, 2, 0, 4, 2, 29, 0], ["findAllOrNullObject", 10, 2, 0, 4, 2, 29, 0], ["getRanges", 10, 2, 0, 4, 1, 29, 0]], [["Activated", 0, 5, "MessageType.worksheetActivatedEvent", "this.id", "_RegisterActivatedEvent", "_UnregisterActivatedEvent"], ["Calculated", 0, 1, "MessageType.worksheetCalculatedEvent", "this.id", "_RegisterCalculatedEvent", "_UnregisterCalculatedEvent"], ["Changed", 1, 5, "MessageType.worksheetDataChangedEvent", "this.id", "_RegisterDataChangedEvent", "_UnregisterDataChangedEvent"], ["ColumnSorted", 0, 11, "MessageType.worksheetColumnSortedEvent", "this.id", "_RegisterColumnSortedEvent", "_UnregisterColumnSortedEvent"], ["Deactivated", 0, 5, "MessageType.worksheetDeactivatedEvent", "this.id", "_RegisterDeactivatedEvent", "_UnregisterDeactivatedEvent"], ["Filtered", 0, 6, "MessageType.worksheetFilteredEvent", "this.id", "_RegisterFilteredEvent", "_UnregisterFilteredEvent"], ["FormatChanged", 1, 2, "MessageType.worksheetFormatChangedEvent", "this.id", "_RegisterFormatChangedEvent", "_UnregisterFormatChangedEvent"], ["RowHiddenChanged", 0, 3, "MessageType.worksheetRowHiddenChangedEvent", "this.id", "_RegisterRowHiddenChangedEvent", "_UnregisterRowHiddenChangedEvent"], ["RowSorted", 0, 11, "MessageType.worksheetRowSortedEvent", "this.id", "_RegisterRowSortedEvent", "_UnregisterRowSortedEvent"], ["SelectionChanged", 0, 5, "MessageType.worksheetSelectionChangedEvent", "this.id", "_RegisterSelectionChangedEvent", "_UnregisterSelectionChangedEvent"], ["SingleClicked", 0, 11, "MessageType.worksheetSingleClickedEvent", "this.id", "_RegisterSingleClickedEvent", "_UnregisterSingleClickedEvent"]]],
			["WorksheetCollection", 1, 0, 0, 0, 40, [], [], [["getCount", 2, 9, 0, 4, 1], ["_RegisterAddedEvent", 0, 5, 0, 0, 0], ["_UnregisterAddedEvent", 0, 5, 0, 0, 0], ["_RegisterActivatedEvent", 2, 5, 0, 0, 0], ["_UnregisterActivatedEvent", 0, 5, 0, 0, 0], ["_RegisterDeactivatedEvent", 0, 5, 0, 0, 0], ["_UnregisterDeactivatedEvent", 0, 5, 0, 0, 0], ["_RegisterDeletedEvent", 0, 5, 0, 0, 0], ["_UnregisterDeletedEvent", 0, 5, 0, 0, 0], ["_RegisterCalculatedEvent", 0, 1, 0, 0, 0], ["_UnregisterCalculatedEvent", 0, 1, 0, 0, 0], ["_RegisterDataChangedEvent", 0, 2, 0, 0, 0], ["_UnregisterDataChangedEvent", 0, 2, 0, 0, 0], ["_RegisterSelectionChangedEvent", 0, 2, 0, 0, 0], ["_UnregisterSelectionChangedEvent", 0, 2, 0, 0, 0], ["addFromBase64", 0, 6, 0, 0, 4], ["_RegisterFilteredEvent", 0, 6, 0, 0, 0], ["_UnregisterFilteredEvent", 0, 6, 0, 0, 0], ["_RegisterFormatChangedEvent", 0, 2, 0, 0, 0], ["_UnregisterFormatChangedEvent", 0, 2, 0, 0, 0], ["_RegisterRowSortedEvent", 0, 11, 0, 0, 0], ["_UnregisterRowSortedEvent", 0, 11, 0, 0, 0], ["_RegisterColumnSortedEvent", 0, 11, 0, 0, 0], ["_UnregisterColumnSortedEvent", 0, 11, 0, 0, 0], ["_RegisterSingleClickedEvent", 0, 11, 0, 0, 0], ["_UnregisterSingleClickedEvent", 0, 11, 0, 0, 0], ["_RegisterRowHiddenChangedEvent", 0, 3, 0, 0, 0], ["_UnregisterRowHiddenChangedEvent", 0, 3, 0, 0, 0]], [["getItem", 18, 0, 0, 4, 1, 40, 0], ["add", 8, 0, 0, 0, 1, 40, 0], ["getActiveWorksheet", 2, 0, 0, 4, 0, 40, 0], ["getItemOrNullObject", 2, 9, 0, 4, 1, 40, 0], ["getFirst", 10, 10, 0, 4, 1, 40, 0], ["getLast", 10, 10, 0, 4, 1, 40, 0]], [["Activated", 0, 5, "MessageType.worksheetActivatedEvent", "OfficeExtension.Constants.eventWorkbookId", "_RegisterActivatedEvent", "_UnregisterActivatedEvent"], ["Added", 0, 5, "MessageType.worksheetAddedEvent", "OfficeExtension.Constants.eventWorkbookId", "_RegisterAddedEvent", "_UnregisterAddedEvent"], ["Calculated", 0, 1, "MessageType.worksheetCalculatedEvent", "OfficeExtension.Constants.eventWorkbookId", "_RegisterCalculatedEvent", "_UnregisterCalculatedEvent"], ["Changed", 1, 2, "MessageType.worksheetDataChangedEvent", "OfficeExtension.Constants.eventWorkbookId", "_RegisterDataChangedEvent", "_UnregisterDataChangedEvent"], ["ColumnSorted", 0, 11, "MessageType.worksheetColumnSortedEvent", "OfficeExtension.Constants.eventWorkbookId", "_RegisterColumnSortedEvent", "_UnregisterColumnSortedEvent"], ["Deactivated", 0, 5, "MessageType.worksheetDeactivatedEvent", "OfficeExtension.Constants.eventWorkbookId", "_RegisterDeactivatedEvent", "_UnregisterDeactivatedEvent"], ["Deleted", 0, 5, "MessageType.worksheetDeletedEvent", "OfficeExtension.Constants.eventWorkbookId", "_RegisterDeletedEvent", "_UnregisterDeletedEvent"], ["Filtered", 0, 6, "MessageType.worksheetFilteredEvent", "OfficeExtension.Constants.eventWorkbookId", "_RegisterFilteredEvent", "_UnregisterFilteredEvent"], ["FormatChanged", 1, 2, "MessageType.worksheetFormatChangedEvent", "OfficeExtension.Constants.eventWorkbookId", "_RegisterFormatChangedEvent", "_UnregisterFormatChangedEvent"], ["RowHiddenChanged", 0, 3, "MessageType.worksheetRowHiddenChangedEvent", "OfficeExtension.Constants.eventWorkbookId", "_RegisterRowHiddenChangedEvent", "_UnregisterRowHiddenChangedEvent"], ["RowSorted", 0, 11, "MessageType.worksheetRowSortedEvent", "OfficeExtension.Constants.eventWorkbookId", "_RegisterRowSortedEvent", "_UnregisterRowSortedEvent"], ["SelectionChanged", 0, 2, "MessageType.worksheetSelectionChangedEvent", "OfficeExtension.Constants.eventWorkbookId", "_RegisterSelectionChangedEvent", "_UnregisterSelectionChangedEvent"], ["SingleClicked", 0, 11, "MessageType.worksheetSingleClickedEvent", "OfficeExtension.Constants.eventWorkbookId", "_RegisterSingleClickedEvent", "_UnregisterSingleClickedEvent"]]],
			["WorksheetProtection", 0, 0, 0, 0, 0, [["protected", 3, 0, 0, 0], ["options", 3, 0, 0, 0]], [], [["protect", 1, 0, 0, 0, 2], ["unprotect", 0, 0, 0, 0, 1]]],
			["WorksheetFreezePanes", 0, 0, 0, 0, 0, [], [], [["unfreeze", 0, 0, 0, 0, 0], ["freezeAt", 0, 0, 0, 0, 1], ["freezeRows", 0, 0, 0, 0, 1], ["freezeColumns", 0, 0, 0, 0, 1]], [["getLocation", 10, 0, 0, 4, 0, 27, 0], ["getLocationOrNullObject", 10, 0, 0, 4, 0, 27, 0]]],
			["Range", 14, 0, 0, 0, 0, [["numberFormat", 5, 0, 0, 0], ["numberFormatLocal", 1, 5, 0, 0], ["values", 5, 0, 0, 0], ["text", 3, 0, 0, 0], ["formulas", 5, 0, 0, 0], ["formulasLocal", 5, 0, 0, 0], ["rowIndex", 3, 0, 0, 0], ["columnIndex", 3, 0, 0, 0], ["rowCount", 3, 0, 0, 0], ["columnCount", 3, 0, 0, 0], ["address", 3, 0, 0, 0], ["addressLocal", 3, 0, 0, 0], ["cellCount", 3, 0, 0, 0], ["_ReferenceId", 2, 0, 0, 0], ["valueTypes", 3, 0, 0, 0], ["formulasR1C1", 5, 7, 0, 0], ["hidden", 3, 7, 0, 0], ["rowHidden", 1, 7, 0, 0], ["columnHidden", 1, 7, 0, 0], ["isEntireColumn", 3, 5, 0, 0], ["isEntireRow", 3, 5, 0, 0], ["hyperlink", 1, 5, 0, 0], ["style", 1, 5, 0, 0], ["linkedDataTypeState", 3, 2, 0, 0], ["hasSpill", 3, 6, 0, 0], ["top", 3, 11, 0, 
ion", 10, 5, 0, 4, 0, 27, 0], ["getAbsoluteResizedRange", 10, 5, 0, 4, 2, 27, 0], ["getTables", 6, 2, 0, 4, 1, 46, 0], ["find", 10, 2, 0, 4, 2, 27, 0], ["findOrNullObject", 10, 2, 0, 4, 2, 27, 0], ["removeDuplicates", 8, 2, 0, 0, 2, 47, 0], ["getSpecialCells", 10, 2, 0, 4, 2, 29, 0], ["getSpecialCellsOrNullObject", 10, 2, 0, 4, 2, 29, 0], ["getSpillingToRange", 10, 6, 0, 4, 0, 27, 0], ["getSpillParent", 10, 6, 0, 4, 0, 27, 0], ["getSpillingToRangeOrNullObject", 10, 6, 0, 4, 0, 27, 0], ["getSpillParentOrNullObject", 10, 6, 0, 4, 0, 27, 0], ["getMergedAreas", 10, 6, 0, 4, 0, 29, 0]]],
			["RangeAreas", 2, 0, 0, 0, 0, [["_ReferenceId", 2, 0, 0, 0], ["address", 3, 0, 0, 0], ["addressLocal", 3, 0, 0, 0], ["areaCount", 3, 0, 0, 0], ["cellCount", 3, 0, 0, 0], ["isEntireColumn", 3, 0, 0, 0], ["isEntireRow", 3, 0, 0, 0], ["style", 1, 0, 0, 0]], [["areas", 19, 0, 0, 4, 0, 48], ["conditionalFormats", 19, 0, 0, 4, 0, 43], ["format", 35, 0, 0, 4, 0, 41], ["dataValidation", 35, 0, 0, 4, 0, 44], ["worksheet", 2, 0, 0, 4, 0, 40]], [["_KeepReference", 2, 0, 0, 0, 0], ["calculate", 0, 0, 0, 0, 0], ["clear", 0, 0, 0, 0, 1], ["setDirty", 0, 0, 0, 0, 0], ["copyFrom", 0, 0, 0, 0, 4], ["convertToLinkedDataType", 0, 0, 0, 0, 2], ["convertDataTypeToText", 0, 0, 0, 0, 0], ["select", 2, 6, 0, 0, 0]], [["getEntireColumn", 10, 0, 0, 4, 0, 29, 0], ["getEntireRow", 10, 0, 0, 4, 0, 29, 0], ["getIntersection", 10, 0, 0, 4, 1, 29, 0], ["getIntersectionOrNullObject", 10, 0, 0, 4, 1, 29, 0], ["getOffsetRangeAreas", 10, 0, 0, 4, 2, 29, 0], ["getUsedRangeAreas", 10, 0, 0, 4, 1, 29, 0], ["getUsedRangeAreasOrNullObject", 10, 0, 0, 4, 1, 29, 0], ["getTables", 6, 0, 0, 4, 1, 46, 0], ["getSpecialCells", 10, 0, 0, 4, 2, 29, 0], ["getSpecialCellsOrNullObject", 10, 0, 0, 4, 2, 29, 0]]],
			["RangeView", 0, 0, 0, 0, 0, [["numberFormat", 1, 0, 0, 0], ["values", 1, 0, 0, 0], ["text", 3, 0, 0, 0], ["formulas", 1, 0, 0, 0], ["formulasLocal", 1, 0, 0, 0], ["formulasR1C1", 1, 0, 0, 0], ["valueTypes", 3, 0, 0, 0], ["rowCount", 3, 0, 0, 0], ["columnCount", 3, 0, 0, 0], ["cellAddresses", 3, 0, 0, 0], ["index", 3, 0, 0, 0]], [["rows", 19, 0, 0, 4, 0, 49]], [], [["getRange", 10, 0, 0, 4, 0, 27, 0]]],
			["RangeViewCollection", 1, 0, 0, 0, 45, [], [], [["getCount", 2, 9, 0, 4, 0]], [["getItemAt", 2, 0, 0, 4, 1, 45, 0]]],
			["SettingCollection", 1, 0, 0, 0, 50, [], [], [["getCount", 2, 0, 0, 4, 0]], [["getItem", 18, 0, 0, 4, 1, 50, 0], ["add", 9, 0, 0, 0, 2, 50, 0], ["getItemOrNullObject", 2, 0, 0, 4, 1, 50, 0]], [], [["SettingsChanged", 1, 0, "_CC.office10EventIdSettingsChangedEvent", "", "", ""]]],
			["Setting", 8, 0, 0, 0, 0, [["key", 3, 0, 0, 0], ["value", 5, 0, 0, 0], ["_Id", 2, 0, 0, 0]], [], [["delete", 0, 0, 0, 0, 0]]],
			["NamedItemCollection", 1, 0, 0, 0, 51, [], [], [["getCount", 2, 9, 0, 4, 0]], [["getItem", 18, 0, 0, 4, 1, 51, 0], ["getItemOrNullObject", 2, 9, 0, 4, 1, 51, 0], ["add", 8, 9, 0, 0, 3, 51, 0], ["addFormulaLocal", 0, 9, 0, 0, 3, 51, 0]]],
			["NamedItem", 0, 0, 0, 0, 0, [["name", 3, 0, 0, 0], ["type", 3, 0, 0, 0], ["value", 3, 0, 0, 0], ["visible", 1, 0, 0, 0], ["_Id", 2, 0, 0, 0], ["comment", 1, 9, 0, 0], ["scope", 3, 9, 0, 0], ["formula", 1, 5, 0, 0]], [["worksheet", 2, 9, 0, 4, 0, 40], ["worksheetOrNullObject", 2, 9, 0, 4, 0, 40], ["arrayValues", 3, 5, 0, 4, 0, 52]], [["delete", 0, 9, 0, 0, 0]], [["getRange", 10, 0, 0, 4, 0, 27, 0], ["getRangeOrNullObject", 10, 9, 0, 4, 0, 27, 0]]],
			["NamedItemArrayValues", 0, 0, 0, 0, 0, [["values", 3, 0, 0, 0], ["types", 3, 0, 0, 0]]],
			["Binding", 0, 0, 0, 0, 0, [["id", 3, 0, 0, 0], ["type", 3, 0, 0, 0]], [], [["getText", 2, 0, 0, 4, 0], ["delete", 0, 8, 0, 0, 0]], [["getTable", 2, 0, 0, 4, 0, 53, 0], ["getRange", 2, 0, 0, 4, 0, 27, 0]], [], [["DataChanged", 1, 8, "_CC.office10EventIdBindingDataChangedEvent", "this.id", "", ""], ["SelectionChanged", 1, 8, "_CC.office10EventIdBindingSelectionChangedEvent", "this.id", "", ""]]],
			["BindingCollection", 1, 0, 0, 0, 54, [["count", 3, 0, 0, 0]], [], [["getCount", 2, 9, 0, 4, 0]], [["getItem", 18, 0, 0, 4, 1, 54, 0], ["getItemAt", 2, 0, 0, 4, 1, 54, 0], ["add", 8, 8, 0, 0, 3, 54, 0], ["addFromNamedItem", 0, 8, 0, 0, 3, 54, 0], ["addFromSelection", 0, 8, 0, 0, 2, 54, 0], ["getItemOrNullObject", 2, 9, 0, 4, 1, 54, 0]]],
			["TableCollection", 5, 0, 0, 0, 53, [["count", 3, 0, 0, 0]], [], [["getCount", 2, 9, 0, 4, 0], ["_RegisterDataChangedEvent", 0, 5, 0, 0, 0], ["_UnregisterDataChangedEvent", 0, 5, 0, 0, 0], ["_RegisterAddedEvent", 0, 2, 0, 0, 0], ["_UnregisterAddedEvent", 0, 2, 0, 0, 0], ["_RegisterDeletedEvent", 0, 2, 0, 0, 0], ["_UnregisterDeletedEvent", 0, 2, 0, 0, 0], ["_RegisterFilteredEvent", 0, 6, 0, 0, 0], ["_UnregisterFilteredEvent", 0, 6, 0, 0, 0]], [["getItem", 18, 0, 0, 4, 1, 53, 0], ["getItemAt", 2, 0, 0, 4, 1, 53, 0], ["add", 8, 0, 0, 0, 2, 53, 0], ["getItemOrNullObject", 2, 9, 0, 4, 1, 53, 0]], [["Added", 0, 2, "MessageType.tableAddedEvent", "this._ParentObject ? this._ParentObject.id : OfficeExtension.Constants.eventWorkbookId", "_RegisterAddedEvent", "_UnregisterAddedEvent"], ["Changed", 1, 5, "MessageType.tableDataChangedEvent", "this._ParentObject ? this._ParentObject.id : OfficeExtension.Constants.eventWorkbookId", "_RegisterDataChangedEvent", "_UnregisterDataChangedEvent"], ["Deleted", 0, 2, "MessageType.tableDeletedEvent", "this._ParentObject ? this._ParentObject.id : OfficeExtension.Constants.eventWorkbookId", "_RegisterDeletedEvent", "_UnregisterDeletedEvent"], ["Filtered", 0, 6, "MessageType.tableFilteredEvent", "this._ParentObject ? this._ParentObject.id : OfficeExtension.Constants.eventWorkbookId", "_RegisterFilteredEvent", "_UnregisterFilteredEvent"]]],
			["TableScopedCollection", 1, 0, 0, 0, 53, [], [], [["getCount", 2, 0, 0, 4, 0]], [["getFirst", 10, 0, 0, 4, 0, 53, 0], ["getItem", 18, 0, 0, 4, 1, 53, 0]]],
			["Table", 24, "workbook.tables", 0, 0, 0, [["id", 3, 0, 0, 0], ["name", 1, 0, 0, 0], ["showHeaders", 1, 0, 0, 0], ["showTotals", 1, 0, 0, 0], ["style", 1, 0, 0, 0], ["highlightFirstColumn", 1, 8, 0, 0], ["highlightLastColumn", 1, 8, 0, 0], ["showBandedRows", 1, 8, 0, 0], ["showBandedColumns", 1, 8, 0, 0], ["showFilterButton", 1, 8, 0, 0], ["legacyId", 3, 1, 0, 0]], [["columns", 19, 0, 0, 4, 0, 55], ["rows", 19, 0, 0, 4, 0, 56], ["sort", 3, 7, 0, 4, 0, 57], ["worksheet", 2, 7, 0, 4, 0, 40], ["autoFilter", 3, 2, 0, 4, 0, 38], ["tableStyle", 35, 15, 0, 4, 0, 58]], [["delete", 0, 0, 0, 0, 0], ["clearFilters", 0, 7, 0, 0, 0], ["reapplyFilters", 0, 7, 0, 0, 0], ["_RegisterSelectionChangedEvent", 0, 5, 0, 0, 0], ["_UnregisterSelectionChangedEvent", 0, 5, 0, 0, 0], ["_RegisterDataChangedEvent", 0, 5, 0, 0, 0], ["_UnregisterDataChangedEvent", 0, 5, 0, 0, 0], ["clearStyle", 0, 6, 0, 0, 0], ["_RegisterFilteredEvent", 0, 6, 0, 0, 0], ["_UnregisterFilteredEvent", 0, 6, 0, 0, 0], ["setStyle", 0, 15, 0, 0, 1]], [["getRange", 10, 0, 0, 4, 0, 27, 0], ["getHeaderRowRange", 10, 0, 0, 4, 0, 27, 0], ["getDataBodyRange", 10, 0, 0, 4, 0, 27, 0], ["getTotalRowRange", 10, 0, 0, 4, 0, 27, 0], ["convertToRange", 8, 7, 0, 0, 0, 27, 0]], [["Changed", 1, 5, "MessageType.tableDataChangedEvent", "this.id.toString()", "_RegisterDataChangedEvent", "_UnregisterDataChangedEvent"], ["Filtered", 0, 6, "MessageType.tableFilteredEvent", "this.id.toString()", "_RegisterFilteredEvent", "_UnregisterFilteredEvent"], ["SelectionChanged", 1, 5, "MessageType.tableSelectionChangedEvent", "this.id", "_RegisterSelectionChangedEvent", "_UnregisterSelectionChangedEvent"]]],
			["TableColumnCollection", 1, 0, 0, 0, 59, [["count", 3, 0, 0, 0]], [], [["getCount", 2, 9, 0, 4, 0]], [["getItem", 18, 0, 0, 4, 1, 59, 0], ["getItemAt", 2, 0, 0, 4, 1, 59, 0], ["add", 8, 0, 0, 0, 3, 59, 0], ["getItemOrNullObject", 2, 9, 0, 4, 1, 59, 0]]],
			["TableColumn", 0, 0, 0, 0, 0, [["id", 3, 0, 0, 0], ["index", 3, 0, 0, 0], ["values", 1, 0, 0, 0], ["name", 1, 0, 0, 0]], [["filter", 3, 7, 0, 4, 0, 60]], [["delete", 0, 0, 0, 0, 0]], [["getRange", 10, 0, 0, 4, 0, 27, 0], ["getHeaderRowRange", 10, 0, 0, 4, 0, 27, 0], ["getDataBodyRange", 10, 0, 0, 4, 0, 27, 0], ["getTotalRowRange", 10, 0, 0, 4, 0, 27, 0]]],
			["TableRowCollection", 1, 0, 0, 0, 61, [["count", 3, 0, 0, 0]], [], [["getCount", 2, 9, 0, 4, 0]], [["getItemAt", 2, 0, 0, 4, 1, 61, 0], ["add", 8, 0, 0, 0, 2, 61, 0]]],
			["TableRow", 0, 0, 0, 0, 0, [["index", 3, 0, 0, 0], ["values", 1, 0, 0, 0]], [], [["delete", 0, 0, 0, 0, 0]], [["getRange", 10, 0, 0, 4, 0, 27, 0]]],
			["DataValidation", 0, 0, 0, 0, 0, [["type", 3, 0, 0, 0], ["rule", 1, 0, 0, 0], ["prompt", 1, 0, 0, 0], ["errorAlert", 1, 0, 0, 0], ["ignoreBlanks", 1, 0, 0, 0], ["valid", 3, 0, 0, 0]], [], [["clear", 0, 0, 0, 0, 0]], [["getInvalidCells", 10, 2, 0, 4, 0, 29, 0], ["getInvalidCellsOrNullObject", 10, 2, 0, 4, 0, 29, 0]]],
			["RemoveDuplicatesResult", 0, 0, 0, 0, 0, [["removed", 3, 0, 0, 0], ["uniqueRemaining", 3, 0, 0, 0]]],
			["RangeFormat", 0, 0, 0, 0, 0, [["wrapText", 1, 0, 0, 0], ["horizontalAlignment", 1, 0, 0, 0], ["verticalAlignment", 1, 0, 0, 0], ["columnWidth", 1, 7, 0, 0], ["rowHeight", 1, 7, 0, 0], ["textOrientation", 1, 5, 0, 0], ["useStandardHeight", 1, 5, 0, 0], ["useStandardWidth", 1, 5, 0, 0], ["readingOrder", 1, 2, 0, 0], ["shrinkToFit", 1, 2, 0, 0], ["indentLevel", 1, 2, 0, 0], ["autoIndent", 1, 2, 0, 0]], [["fill", 35, 0, 0, 4, 0, 62], ["font", 35, 0, 0, 4, 0, 63], ["borders", 51, 0, 0, 4, 0, 64], ["protection", 35, 7, 0, 4, 0, 65]], [["autofitColumns", 0, 7, 0, 0, 0], ["autofitRows", 0, 7, 0, 0, 0], ["adjustIndent", 0, 14, 0, 0, 1]]],
			["FormatProtection", 0, 0, 0, 0, 0, [["locked", 1, 0, 0, 0], ["formulaHidden", 1, 0, 0, 0]]],
			["RangeFill", 0, 0, 0, 0, 0, [["color", 1, 0, 0, 0], ["tintAndShade", 1, 2, 0, 0], ["patternTintAndShade", 1, 2, 0, 0], ["pattern", 1, 2, 0, 0], ["patternColor", 1, 2, 0, 0]], [], [["clear", 0, 0, 0, 0, 0]]],
			["RangeBorder", 0, 0, 0, 0, 0, [["sideIndex", 3, 0, 0, 0], ["style", 1, 0, 0, 0], ["weight", 1, 0, 0, 0], ["color", 1, 0, 0, 0], ["tintAndShade", 1, 2, 0, 0]]],
			["RangeBorderCollection", 1, 0, 0, 0, 66, [["count", 3, 0, 0, 0], ["tintAndShade", 1, 2, 0, 0]], [], [], [["getItem", 18, 0, 0, 4, 1, 66, 0], ["getItemAt", 2, 0, 0, 4, 1, 66, 0]]],
			["RangeFont", 0, 0, 0, 0, 0, [["name", 1, 0, 0, 0], ["size", 1, 0, 0, 0], ["color", 1, 0, 0, 0], ["italic", 1, 0, 0, 0], ["bold", 1, 0, 0, 0], ["underline", 1, 0, 0, 0], ["strikethrough", 1, 2, 0, 0], ["subscript", 1, 2, 0, 0], ["superscript", 1, 2, 0, 0], ["tintAndShade", 1, 2, 0, 0]]],
			["ChartCollection", 5, 0, 0, 0, 28, [["count", 3, 0, 0, 0]], [], [["getCount", 2, 9, 0, 4, 0], ["_RegisterAddedEvent", 0, 1, 0, 0, 0], ["_UnregisterAddedEvent", 0, 1, 0, 0, 0], ["_RegisterActivatedEvent", 0, 1, 0, 0, 0], ["_UnregisterActivatedEvent", 0, 1, 0, 0, 0], ["_RegisterDeactivatedEvent", 0, 1, 0, 0, 0], ["_UnregisterDeactivatedEvent", 0, 1, 0, 0, 0], ["_RegisterDeletedEvent", 0, 1, 0, 0, 0], ["_UnregisterDeletedEvent", 0, 1, 0, 0, 0]], [["add", 9, 0, 0, 0, 3, 28, 0], ["getItemAt", 2, 0, 0, 4, 1, 28, 0], ["_GetItem", 18, 0, 0, 4, 1, 28, 0], ["getItem", 2, 0, 0, 4, 1, 28, 0], ["getItemOrNullObject", 2, 9, 0, 4, 1, 28, 0]], [["Activated", 0, 1, "MessageType.chartActivatedEvent", "this._ParentObject.id", "_RegisterActivatedEvent", "_UnregisterActivatedEvent"], ["Added", 0, 1, "MessageType.chartAddedEvent", "this._ParentObject.id", "_RegisterAddedEvent", "_UnregisterAddedEvent"], ["Deactivated", 0, 1, "MessageType.chartDeactivatedEvent", "this._ParentObject.id", "_RegisterDeactivatedEvent", "_UnregisterDeactivatedEvent"], ["Deleted", 0, 1, "MessageType.chartDeletedEvent", "this._ParentObject.id", "_RegisterDeletedEvent", "_UnregisterDeletedEvent"]]],
			["Chart", 0, 0, 0, 0, 0, [["name", 1, 0, 0, 0], ["top", 1, 0, 0, 0], ["left", 1, 0, 0, 0], ["width", 1, 0, 0, 0], ["height", 1, 0, 0, 0], ["id", 3, 5, 0, 0], ["showAllFieldButtons", 1, 5, 0, 0], ["chartType", 1, 5, 0, 0], ["showDataLabelsOverMaximum", 1, 1, 0, 0], ["categoryLabelLevel", 1, 1, 0, 0], ["style", 1, 1, 0, 0], ["displayBlanksAs", 1, 1, 0, 0], ["plotBy", 1, 1, 0, 0], ["plotVisibleOnly", 1, 1, 0, 0], ["seriesNameLevel", 1, 1, 0, 0]], [["title", 35, 0, 0, 4, 0, 67], ["dataLabels", 35, 0, 0, 4, 0, 68], ["legend", 35, 0, 0, 4, 0, 69], ["series", 19, 0, 0, 4, 0, 70], ["axes", 35, 0, 0, 4, 0, 71], ["format", 35, 0, 0, 4, 0, 72], ["worksheet", 2, 7, 0, 4, 0, 40], ["plotArea", 35, 1, 0, 4, 0, 73], ["pivotOptions", 35, 2, 0, 4, 0, 74]], [["setData", 1, 0, 0, 0, 2], ["delete", 0, 0, 0, 0, 0], ["setPosition", 0, 0, 0, 0, 2], ["getImage", 2, 7, 0, 4, 3], ["_RegisterActivatedEvent", 0, 1, 0, 0, 0], ["_UnregisterActivatedEvent", 0, 1, 0, 0, 0], ["_RegisterDeactivatedEvent", 0, 1, 0, 0, 0], ["_UnregisterDeactivatedEvent", 0, 1, 0, 0, 0], ["activate", 2, 2, 0, 0, 0]], [], [["Activated", 0, 1, "MessageType.chartActivatedEvent", "this.id", "_RegisterActivatedEvent", "_UnregisterActivatedEvent"], ["Deactivated", 0, 1, "MessageType.chartDeactivatedEvent", "this.id", "_RegisterDeactivatedEvent", "_UnregisterDeactivatedEvent"]]],
			["ChartPivotOptions", 0, 0, 0, 0, 0, [["showAxisFieldButtons", 1, 0, 0, 0], ["showLegendFieldButtons", 1, 0, 0, 0], ["showReportFilterFieldButtons", 1, 0, 0, 0], ["showValueFieldButtons", 1, 0, 0, 0]]],
			["ChartAreaFormat", 0, 0, 0, 0, 0, [["roundedCorners", 1, 2, 0, 0], ["colorScheme", 1, 2, 0, 0]], [["fill", 3, 0, 0, 4, 0, 75], ["font", 35, 0, 0, 4, 0, 76], ["border", 35, 5, 0, 4, 0, 77]]],
			["ChartSeriesCollection", 1, 0, 0, 0, 78, [["count", 3, 0, 0, 0]], [], [["getCount", 2, 9, 0, 4, 0]], [["getItemAt", 2, 0, 0, 4, 1, 78, 0], ["add", 8, 5, 0, 0, 2, 78, 0]]],
			["ChartDataLabels", 0, 0, 0, 0, 0, [["position", 1, 0, 0, 0], ["showValue", 1, 0, 0, 0], ["showSeriesName", 1, 0, 0, 0], ["showCategoryName", 1, 0, 0, 0], ["showLegendKey", 1, 0, 0, 0], ["showPercentage", 1, 0, 0, 0], ["showBubbleSize", 1, 0, 0, 0], ["separator", 1, 0, 0, 0], ["numberFormat", 1, 1, 0, 0], ["linkNumberFormat", 1, 2, 0, 0], ["textOrientation", 1, 1, 0, 0], ["autoText", 1, 1, 0, 0], ["horizontalAlignment", 1, 1, 0, 0], ["verticalAlignment", 1, 1, 0, 0]], [["format", 35, 0, 0, 4, 0, 95]]],
			["ChartDataLabel", 0, 0, 0, 0, 0, [["position", 1, 0, 0, 0], ["showValue", 1, 0, 0, 0], ["showSeriesName", 1, 0, 0, 0], ["showCategoryName", 1, 0, 0, 0], ["showLegendKey", 1, 0, 0, 0], ["showPercentage", 1, 0, 0, 0], ["showBubbleSize", 1, 0, 0, 0], ["separator", 1, 0, 0, 0], ["top", 1, 1, 0, 0], ["left", 1, 1, 0, 0], ["width", 3, 1, 0, 0], ["height", 3, 1, 0, 0], ["formula", 1, 1, 0, 0], ["textOrientation", 1, 1, 0, 0], ["horizontalAlignment", 1, 1, 0, 0], ["verticalAlignment", 1, 1, 0, 0], ["text", 1, 1, 0, 0], ["autoText", 1, 1, 0, 0], ["numberFormat", 1, 1, 0, 0], ["linkNumberFormat", 1, 2, 0, 0]], [["format", 35, 1, 0, 4, 0, 95]]],
			["ChartDataLabelFormat", 0, 0, 0, 0, 0, [], [["font", 35, 0, 0, 4, 0, 76], ["fill", 3, 0, 0, 4, 0, 75], ["border", 35, 1, 0, 4, 0, 77]]],
			["ChartErrorBars", 0, 0, 0, 0, 0, [["endStyleCap", 1, 0, 0, 0], ["include", 1, 0, 0, 0], ["type", 1, 0, 0, 0], ["visible", 1, 0, 0, 0]], [["format", 35, 0, 0, 4, 0, 96]]],
			["ChartErrorBarsFormat", 0, 0, 0, 0, 0, [], [["line", 35, 0, 0, 4, 0, 86]]],
			["ChartGridlines", 0, 0, 0, 0, 0, [["visible", 1, 0, 0, 0]], [["format", 35, 0, 0, 4, 0, 97]]],
			["ChartGridlinesFormat", 0, 0, 0, 0, 0, [], [["line", 35, 0, 0, 4, 0, 86]]],
			["ChartLegend", 0, 0, 0, 0, 0, [["visible", 1, 0, 0, 0], ["position", 1, 0, 0, 0], ["overlay", 1, 0, 0, 0], ["left", 1, 5, 0, 0], ["top", 1, 5, 0, 0], ["width", 1, 5, 0, 0], ["height", 1, 5, 0, 0], ["showShadow", 1, 5, 0, 0]], [["format", 35, 0, 0, 4, 0, 98], ["legendEntries", 19, 5, 0, 4, 0, 99]]],
			["ChartLegendEntry", 0, 0, 0, 0, 0, [["visible", 1, 0, 0, 0], ["left", 3, 1, 0, 0], ["top", 3, 1, 0, 0], ["width", 3, 1, 0, 0], ["height", 3, 1, 0, 0], ["index", 3, 1, 0, 0]]],
			["ChartLegendEntryCollection", 1, 0, 0, 0, 100, [], [], [["getCount", 2, 0, 0, 4, 0]], [["getItemAt", 2, 0, 0, 4, 1, 100, 0]]],
			["ChartLegendFormat", 0, 0, 0, 0, 0, [], [["font", 35, 0, 0, 4, 0, 76], ["fill", 3, 0, 0, 4, 0, 75], ["border", 35, 1, 0, 4, 0, 77]]],
			["ChartMapOptions", 0, 0, 0, 0, 0, [["level", 1, 0, 0, 0], ["labelStrategy", 1, 0, 0, 0], ["projectionType", 1, 0, 0, 0]]],
			["ChartTitle", 0, 0, 0, 0, 0, [["visible", 1, 0, 0, 0], ["text", 1, 0, 0, 0], ["overlay", 1, 0, 0, 0], ["horizontalAlignment", 1, 5, 0, 0], ["top", 1, 5, 0, 0], ["left", 1, 5, 0, 0], ["width", 3, 5, 0, 0], ["height", 3, 5, 0, 0], ["verticalAlignment", 1, 5, 0, 0], ["textOrientation", 1, 5, 0, 0], ["position", 1, 5, 0, 0], ["showShadow", 1, 5, 0, 0]], [["format", 35, 0, 0, 4, 0, 101]], [["setFormula", 0, 5, 0, 0, 1]], [["getSubstring", 2, 5, 0, 4, 2, 102, 0]]],
			["ChartFormatString", 0, 0, 0, 0, 0, [], [["font", 35, 0, 0, 4, 0, 76]]],
			["ChartTitleFormat", 0, 0, 0, 0, 0, [], [["font", 35, 0, 0, 4, 0, 76], ["fill", 3, 0, 0, 4, 0, 75], ["border", 35, 5, 0, 4, 0, 77]]],
			["ChartFill", 4, 0, 0, 0, 0, [], [], [["setSolidColor", 0, 0, 0, 0, 1], ["clear", 0, 0, 0, 0, 0]]],
			["ChartBorder", 0, 0, 0, 0, 0, [["color", 1, 0, 0, 0], ["lineStyle", 1, 0, 0, 0], ["weight", 1, 0, 0, 0]], [], [["clear", 0, 1, 0, 0, 0]]],
			["ChartBinOptions", 0, 0, 0, 0, 0, [["type", 1, 0, 0, 0], ["width", 1, 0, 0, 0], ["count", 1, 0, 0, 0], ["allowOverflow", 1, 0, 0, 0], ["allowUnderflow", 1, 0, 0, 0], ["overflowValue", 1, 0, 0, 0], ["underflowValue", 1, 0, 0, 0]]],
			["ChartBoxwhiskerOptions", 0, 0, 0, 0, 0, [["showInnerPoints", 1, 0, 0, 0], ["showOutlierPoints", 1, 0, 0, 0], ["showMeanMarker", 1, 0, 0, 0], ["showMeanLine", 1, 0, 0, 0], ["quartileCalculation", 1, 0, 0, 0]]],
			["ChartLineFormat", 0, 0, 0, 0, 0, [["color", 1, 0, 0, 0], ["lineStyle", 1, 5, 0, 0], ["weight", 1, 5, 0, 0]], [], [["clear", 0, 0, 0, 0, 0]]],
			["ChartFont", 0, 0, 0, 0, 0, [["bold", 1, 0, 0, 0], ["color", 1, 0, 0, 0], ["italic", 1, 0, 0, 0], ["name", 1, 0, 0, 0], ["size", 1, 0, 0, 0], ["underline", 1, 0, 0, 0]]],
			["ChartTrendline", 0, 0, 0, 0, 0, [["type", 1, 0, 0, 0], ["polynomialOrder", 1, 0, 0, 0], ["movingAveragePeriod", 1, 0, 0, 0], ["_Id", 2, 0, 0, 0], ["showEquation", 1, 1, 0, 0], ["showRSquared", 1, 1, 0, 0], ["forwardPeriod", 1, 1, 0, 0], ["backwardPeriod", 1, 1, 0, 0], ["name", 1, 0, 0, 0], ["intercept", 1, 0, 0, 0]], [["format", 35, 0, 0, 4, 0, 103], ["label", 35, 1, 0, 4, 0, 104]], [["delete", 0, 0, 0, 0, 0]]],
			["ChartTrendlineCollection", 1, 0, 0, 0, 105, [], [], [["getCount", 2, 0, 0, 4, 0]], [["add", 8, 0, 0, 0, 1, 105, 0], ["getItem", 18, 0, 0, 4, 1, 105, 0]]],
			["ChartTrendlineFormat", 0, 0, 0, 0, 0, [], [["line", 35, 0, 0, 4, 0, 86]]],
			["ChartTrendlineLabel", 0, 0, 0, 0, 0, [["top", 1, 0, 0, 0], ["left", 1, 0, 0, 0], ["width", 3, 0, 0, 0], ["height", 3, 0, 0, 0], ["formula", 1, 0, 0, 0], ["textOrientation", 1, 0, 0, 0], ["horizontalAlignment", 1, 0, 0, 0], ["verticalAlignment", 1, 0, 0, 0], ["text", 1, 0, 0, 0], ["autoText", 1, 0, 0, 0], ["numberFormat", 1, 0, 0, 0], ["linkNumberFormat", 1, 2, 0, 0]], [["format", 35, 0, 0, 4, 0, 106]]],
			["ChartTrendlineLabelFormat", 0, 0, 0, 0, 0, [], [["fill", 3, 0, 0, 4, 0, 75], ["border", 35, 0, 0, 4, 0, 77], ["font", 35, 0, 0, 4, 0, 76]]],
			["ChartPlotArea", 0, 0, 0, 0, 0, [["left", 1, 0, 0, 0], ["top", 1, 0, 0, 0], ["width", 1, 0, 0, 0], ["height", 1, 0, 0, 0], ["insideLeft", 1, 0, 0, 0], ["insideTop", 1, 0, 0, 0], ["insideWidth", 1, 0, 0, 0], ["insideHeight", 1, 0, 0, 0], ["position", 1, 0, 0, 0]], [["format", 35, 0, 0, 4, 0, 107]]],
			["ChartPlotAreaFormat", 0, 0, 0, 0, 0, [], [["border", 35, 0, 0, 4, 0, 77], ["fill", 3, 0, 0, 4, 0, 75]]],
			["VisualCollection", 5, 0, 0, 0, 108, [], [], [["getDefinitions", 2, 0, 0, 4, 0], ["getPreview", 2, 0, 0, 4, 4], ["bootstrapAgaveVisual", 0, 0, 0, 2, 0], ["getCount", 2, 0, 0, 4, 0], ["_RegisterSelectionChangedEvent", 2, 0, 0, 0, 0], ["_UnregisterSelectionChangedEvent", 2, 0, 0, 0, 0]], [["add", 8, 0, 0, 2, 3, 108, 0], ["_GetItem", 18, 0, 0, 4, 1, 108, 0], ["getSelectedOrNullObject", 2, 0, 0, 4, 0, 108, 0]], [["AgaveVisualUpdate", 1, 0, "MessageType.agaveVisualUpdateEvent", "", "null", "null"], ["SelectionChanged", 0, 0, "MessageType.visualSelectionChangedEvent", "this._ParentObject.id", "_RegisterSelectionChangedEvent", "_UnregisterSelectionChangedEvent"]]],
			["Visual", 0, 0, 0, 0, 0, [["id", 3, 0, 0, 0], ["isSupportedInVisualTaskpane", 3, 0, 0, 0]], [["properties", 18, 0, 0, 4, 0, 109]], [["delete", 0, 0, 0, 2, 0], ["getProperty", 2, 0, 0, 4, 1], ["setProperty", 0, 0, 0, 2, 2], ["changeDataSource", 0, 0, 0, 2, 2], ["getDataSource", 2, 0, 0, 4, 0], ["setPropertyToDefault", 0, 0, 0, 2, 1], ["_RegisterChangeEvent", 2, 0, 0, 0, 0], ["_UnregisterChangeEvent", 2, 0, 0, 0, 0], ["serializeProperties", 2, 0, 0, 4, 0], ["deserializeProperties", 0, 0, 0, 0, 1]], [["getChildProperties", 6, 0, 0, 4, 2, 109, 0], ["getDataControllerClient", 2, 0, 0, 4, 0, 110, 0], ["getElementChildProperties", 6, 0, 0, 4, 3, 109, 0]], [["ChangeNotification", 1, 0, "MessageType.visualChangeEvent", "this.id", "_RegisterChangeEvent", "_UnregisterChangeEvent"]]],
			["VisualProperty", 0, 0, 0, 0, 0, [["type", 3, 0, 0, 0], ["value", 3, 0, 0, 0], ["id", 3, 0, 0, 0], ["localizedName", 3, 0, 0, 0], ["options", 3, 0, 0, 0], ["localizedOptions", 3, 0, 0, 0], ["hasDefault", 3, 0, 0, 0], ["isDefault", 3, 0, 0, 0], ["min", 3, 0, 0, 0], ["max", 3, 0, 0, 0], ["stepSize", 3, 0, 0, 0], ["hideMeButShowChildrenUI", 3, 0, 0, 0], ["expandableUI", 3, 0, 0, 0], ["nextPropOnSameLine", 3, 0, 0, 0], ["showResetUI", 3, 0, 0, 0]], [], [["getBoolMetaProperty", 2, 0, 0, 4, 1]]],
			["VisualPropertyCollection", 1, 0, 0, 0, 111, [], [], [["getCount", 2, 0, 0, 4, 0]], [["getItem", 18, 0, 0, 4, 1, 111, 0], ["getItemAt", 2, 0, 0, 4, 1, 111, 0]]],
			["DataControllerClient", 0, 0, 0, 0, 0, [], [], [["getWells", 2, 0, 0, 4, 0], ["getAssociatedFields", 2, 0, 0, 4, 1], ["getAvailableFields", 2, 0, 0, 4, 1], ["addField", 0, 0, 0, 2, 3], ["removeField", 0, 0, 0, 2, 2], ["moveField", 0, 0, 0, 2, 3]]],
			["RangeSort", 0, 0, 0, 0, 0, [], [], [["apply", 0, 0, 0, 0, 5]]],
			["TableSort", 0, 0, 0, 0, 0, [["matchCase", 3, 0, 0, 0], ["method", 3, 0, 0, 0], ["fields", 3, 0, 0, 0]], [], [["apply", 0, 0, 0, 0, 3], ["clear", 0, 0, 0, 0, 0], ["reapply", 0, 0, 0, 0, 0]]],
			["Filter", 0, 0, 0, 0, 0, [["criteria", 3, 0, 0, 0]], [], [["apply", 0, 0, 0, 0, 1], ["clear", 0, 0, 0, 0, 0], ["applyBottomItemsFilter", 0, 0, 0, 0, 1], ["applyBottomPercentFilter", 0, 0, 0, 0, 1], ["applyCellColorFilter", 0, 0, 0, 0, 1], ["applyDynamicFilter", 0, 0, 0, 0, 1], ["applyFontColorFilter", 0, 0, 0, 0, 1], ["applyValuesFilter", 0, 0, 0, 0, 1], ["applyTopItemsFilter", 0, 0, 0, 0, 1], ["applyTopPercentFilter", 0, 0, 0, 0, 1], ["applyIconFilter", 0, 0, 0, 0, 1], ["applyCustomFilter", 0, 0, 0, 0, 3]]],
			["AutoFilter", 0, 0, 0, 0, 0, [["enabled", 3, 0, 0, 0], ["isDataFiltered", 3, 0, 0, 0], ["criteria", 3, 0, 0, 0]], [], [["apply", 0, 0, 0, 0, 3], ["reapply", 0, 0, 0, 0, 0], ["remove", 0, 0, 0, 0, 0], ["clearCriteria", 0, 0, 0, 0, 0]], [["getRange", 10, 0, 0, 4, 0, 27, 0], ["getRangeOrNullObject", 10, 0, 0, 4, 0, 27, 0]]],
			["CultureInfo", 0, 0, 0, 0, 0, [["name", 3, 0, 0, 0]], [["numberFormat", 3, 0, 0, 4, 0, 112]]],
			["NumberFormatInfo", 0, 0, 0, 0, 0, [["numberDecimalSeparator", 3, 0, 0, 0], ["numberGroupSeparator", 3, 0, 0, 0]]],
			["CustomXmlPartScopedCollection", 1, 0, 0, 0, 113, [], [], [["getCount", 2, 0, 0, 4, 0]], [["getItem", 18, 0, 0, 4, 1, 113, 0], ["getItemOrNullObject", 2, 0, 0, 4, 1, 113, 0], ["getOnlyItem", 2, 0, 0, 4, 0, 113, 0], ["getOnlyItemOrNullObject", 2, 0, 0, 4, 0, 113, 0]]],
			["CustomXmlPartCollection", 1, 0, 0, 0, 113, [], [], [["getCount", 2, 0, 0, 4, 0]], [["getItem", 18, 0, 0, 4, 1, 113, 0], ["add", 8, 0, 0, 0, 1, 113, 0], ["getByNamespace", 6, 0, 0, 4, 1, 114, 0], ["getItemOrNullObject", 2, 0, 0, 4, 1, 113, 0]]],
			["CustomXmlPart", 0, 0, 0, 0, 0, [["id", 3, 0, 0, 0], ["namespaceUri", 3, 0, 0, 0]], [], [["delete", 0, 0, 0, 0, 0], ["getXml", 2, 0, 0, 4, 0], ["setXml", 0, 0, 0, 0, 1]]],
			["_V1Api", 0, 0, 0, 0, 0, [], [], [["bindingGetData", 2, 0, 0, 4, 1], ["getSelectedData", 2, 0, 0, 4, 1], ["gotoById", 2, 0, 0, 4, 1], ["bindingAddFromSelection", 2, 0, 0, 0, 1], ["bindingGetById", 2, 0, 0, 4, 1], ["bindingReleaseById", 2, 0, 0, 0, 1], ["bindingGetAll", 2, 0, 0, 4, 0], ["bindingAddFromNamedItem", 2, 0, 0, 0, 1], ["bindingAddFromPrompt", 2, 0, 0, 0, 1], ["bindingDeleteAllDataValues", 0, 0, 0, 0, 1], ["setSelectedData", 0, 0, 0, 0, 1], ["bindingClearFormats", 0, 0, 0, 0, 1], ["bindingSetData", 0, 0, 0, 0, 1], ["bindingSetFormats", 0, 0, 0, 0, 1], ["bindingSetTableOptions", 0, 0, 0, 0, 1], ["bindingAddRows", 0, 0, 0, 0, 1], ["bindingAddColumns", 0, 0, 0, 0, 1], ["getFilePropertiesAsync", 2, 4, 0, 4, 0]]],
			["PivotTableCollection", 1, 0, 0, 0, 115, [], [], [["refreshAll", 0, 0, 0, 0, 0], ["getCount", 2, 9, 0, 4, 0]], [["getItem", 18, 0, 0, 4, 1, 115, 0], ["getItemOrNullObject", 2, 9, 0, 4, 1, 115, 0], ["add", 8, 1, 0, 0, 3, 115, 0]]],
			["PivotTable", 0, 0, 0, 0, 0, [["name", 1, 0, 0, 0], ["id", 3, 10, 0, 0], ["useCustomSortLists", 1, 2, 0, 0], ["enableDataValueEditing", 1, 2, 0, 0]], [["worksheet", 2, 0, 0, 4, 0, 40], ["hierarchies", 19, 1, 0, 4, 0, 116], ["rowHierarchies", 19, 1, 0, 4, 0, 117], ["columnHierarchies", 19, 1, 0, 4, 0, 117], ["dataHierarchies", 19, 1, 0, 4, 0, 118], ["filterHierarchies", 19, 1, 0, 4, 0, 119], ["layout", 2, 1, 0, 4, 0, 120]], [["refresh", 0, 0, 0, 0, 0], ["delete", 0, 1, 0, 0, 0]]],
			["PivotLayout", 0, 0, 0, 0, 0, [["showColumnGrandTotals", 1, 0, 0, 0], ["showRowGrandTotals", 1, 0, 0, 0], ["enableFieldList", 1, 11, 0, 0], ["subtotalLocation", 1, 0, 0, 0], ["layoutType", 1, 0, 0, 0], ["autoFormat", 1, 2, 0, 0], ["preserveFormatting", 1, 2, 0, 0]], [["pivotStyle", 35, 15, 0, 4, 0, 121]], [["getPivotItems", 0, 2, 0, 0, 2], ["setAutoSortOnCell", 0, 2, 0, 0, 2], ["setStyle", 0, 15, 0, 0, 1]], [["getRange", 0, 0, 0, 0, 0, 27, 0], ["getRowLabelRange", 0, 0, 0, 0, 0, 27, 0], ["getColumnLabelRange", 0, 0, 0, 0, 0, 27, 0], ["getFilterAxisRange", 0, 0, 0, 0, 0, 27, 0], ["getDataBodyRange", 0, 0, 0, 0, 0, 27, 0], ["getCell", 0, 6, 0, 0, 3, 27, 0], ["getDataHierarchy", 0, 2, 0, 0, 1, 122, 0]]],
			["PivotHierarchyCollection", 1, 0, 0, 0, 123, [], [], [["getCount", 2, 0, 0, 4, 0]], [["getItem", 18, 0, 0, 4, 1, 123, 0], ["getItemOrNullObject", 2, 0, 0, 4, 1, 123, 0]]],
			["PivotHierarchy", 0, 0, 0, 0, 0, [["id", 3, 0, 0, 0], ["name", 1, 0, 0, 0]], [["fields", 19, 0, 0, 4, 0, 124]]],
			["RowColumnPivotHierarchyCollection", 1, 0, 0, 0, 125, [], [], [["getCount", 2, 0, 0, 4, 0], ["remove", 0, 0, 0, 0, 1]], [["getItem", 18, 0, 0, 4, 1, 125, 0], ["getItemOrNullObject", 2, 0, 0, 4, 1, 125, 0], ["add", 8, 0, 0, 0, 1, 125, 0]]],
			["RowColumnPivotHierarchy", 0, 0, 0, 0, 0, [["id", 3, 0, 0, 0], ["name", 1, 0, 0, 0], ["position", 1, 0, 0, 0]], [["fields", 19, 0, 0, 4, 0, 124]], [["setToDefault", 0, 0, 0, 0, 0]]],
			["FilterPivotHierarchyCollection", 1, 0, 0, 0, 126, [], [], [["getCount", 2, 0, 0, 4, 0], ["remove", 0, 0, 0, 0, 1]], [["getItem", 18, 0, 0, 4, 1, 126, 0], ["getItemOrNullObject", 2, 0, 0, 4, 1, 126, 0], ["add", 8, 0, 0, 0, 1, 126, 0]]],
			["FilterPivotHierarchy", 0, 0, 0, 0, 0, [["id", 3, 0, 0, 0], ["name", 1, 0, 0, 0], ["position", 1, 0, 0, 0], ["enableMultipleFilterItems", 1, 0, 0, 0]], [["fields", 19, 0, 0, 4, 0, 124]], [["setToDefault", 0, 0, 0, 0, 0]]],
			["DataPivotHierarchyCollection", 1, 0, 0, 0, 122, [], [], [["getCount", 2, 0, 0, 4, 0], ["remove", 0, 0, 0, 0, 1]], [["getItem", 18, 0, 0, 4, 1, 122, 0], ["getItemOrNullObject", 2, 0, 0, 4, 1, 122, 0], ["add", 8, 0, 0, 0, 1, 122, 0]]],
			["DataPivotHierarchy", 0, 0, 0, 0, 0, [["id", 3, 0, 0, 0], ["name", 1, 0, 0, 0], ["position", 1, 0, 0, 0], ["numberFormat", 1, 0, 0, 0], ["summarizeBy", 1, 0, 0, 0], ["showAs", 1, 0, 0, 0]], [["field", 35, 0, 0, 4, 0, 127]], [["setToDefault", 0, 0, 0, 0, 0]]],
			["PivotFieldCollection", 1, 0, 0, 0, 127, [], [], [["getCount", 2, 0, 0, 4, 0]], [["getItem", 18, 0, 0, 4, 1, 127, 0], ["getItemOrNullObject", 2, 0, 0, 4, 1, 127, 0]]],
			["PivotField", 0, 0, 0, 0, 0, [["id", 3, 0, 0, 0], ["name", 1, 0, 0, 0], ["subtotals", 1, 0, 0, 0], ["showAllItems", 1, 0, 0, 0]], [["items", 19, 0, 0, 4, 0, 128]], [["sortByLabels", 1, 0, 0, 0, 1], ["sortByValues", 0, 2, 0, 0, 3]]],
			["PivotItemCollection", 1, 0, 0, 0, 129, [], [], [["getCount", 2, 0, 0, 4, 0]], [["getItem", 18, 0, 0, 4, 1, 129, 0], ["getItemOrNullObject", 2, 0, 0, 4, 1, 129, 0]]],
			["PivotItem", 0, 0, 0, 0, 0, [["id", 3, 0, 0, 0], ["name", 1, 0, 0, 0], ["isExpanded", 1, 0, 0, 0], ["visible", 1, 0, 0, 0]]],
			["WorksheetCustomProperty", 0, 0, 0, 0, 0, [["key", 3, 0, 0, 0], ["value", 3, 0, 0, 0]]],
			["WorksheetCustomPropertyCollection", 1, 0, 0, 0, 130, [], [], [["getCount", 2, 0, 0, 4, 0]], [["getItem", 18, 0, 0, 4, 1, 130, 0], ["getItemOrNullObject", 0, 0, 0, 0, 1, 130, 0]]],
			["DocumentProperties", 0, 0, 0, 0, 0, [["title", 1, 0, 0, 0], ["subject", 1, 0, 0, 0], ["author", 1, 0, 0, 0], ["keywords", 1, 0, 0, 0], ["comments", 1, 0, 0, 0], ["lastAuthor", 3, 0, 0, 0], ["revisionNumber", 1, 0, 0, 0], ["creationDate", 11, 0, 0, 0], ["category", 1, 0, 0, 0], ["manager", 1, 0, 0, 0], ["company", 1, 0, 0, 0]], [["custom", 19, 0, 0, 4, 0, 131]]],
			["CustomProperty", 0, 0, 0, 0, 0, [["key", 3, 0, 0, 0], ["value", 1, 0, 0, 0], ["type", 3, 0, 0, 0]], [], [["delete", 0, 0, 0, 0, 0]]],
			["CustomPropertyCollection", 1, 0, 0, 0, 132, [], [], [["getCount", 2, 0, 0, 4, 0], ["deleteAll", 0, 0, 0, 0, 0]], [["getItemOrNullObject", 2, 0, 0, 4, 1, 132, 0], ["add", 8, 0, 0, 0, 2, 132, 0], ["getItem", 18, 0, 0, 4, 1, 132, 0]]],
			["ColorScaleConditionalFormat", 0, 0, 0, 0, 0, [["threeColorScale", 3, 0, 0, 0], ["criteria", 1, 0, 0, 0]]],
			["TopBottomConditionalFormat", 0, 0, 0, 0, 0, [["rule", 1, 0, 0, 0]], [["format", 35, 0, 0, 4, 0, 145]]],
			["PresetCriteriaConditionalFormat", 0, 0, 0, 0, 0, [["rule", 1, 0, 0, 0]], [["format", 35, 0, 0, 4, 0, 145]]],
			["TextConditionalFormat", 0, 0, 0, 0, 0, [["rule", 1, 0, 0, 0]], [["format", 35, 0, 0, 4, 0, 145]]],
			["CellValueConditionalFormat", 0, 0, 0, 0, 0, [["rule", 1, 0, 0, 0]], [["format", 35, 0, 0, 4, 0, 145]]],
			["ConditionalRangeFormat", 0, 0, 0, 0, 0, [["numberFormat", 1, 0, 0, 0]], [["fill", 35, 0, 0, 4, 0, 146], ["font", 35, 0, 0, 4, 0, 147], ["borders", 51, 0, 0, 4, 0, 148]]],
			["ConditionalRangeFont", 0, 0, 0, 0, 0, [["color", 1, 0, 0, 0], ["italic", 1, 0, 0, 0], ["bold", 1, 0, 0, 0], ["underline", 1, 0, 0, 0], ["strikethrough", 1, 0, 0, 0]], [], [["clear", 0, 0, 0, 0, 0]]],
			["ConditionalRangeFill", 0, 0, 0, 0, 0, [["color", 1, 0, 0, 0]], [], [["clear", 0, 0, 0, 0, 0]]],
			["ConditionalRangeBorder", 0, 0, 0, 0, 0, [["sideIndex", 3, 0, 0, 0], ["style", 1, 0, 0, 0], ["color", 1, 0, 0, 0]]],
			["ConditionalRangeBorderCollection", 1, 0, 0, 0, 149, [["count", 3, 0, 0, 0]], [["top", 35, 0, 0, 4, 0, 149], ["bottom", 35, 0, 0, 4, 0, 149], ["left", 35, 0, 0, 4, 0, 149], ["right", 35, 0, 0, 4, 0, 149]], [], [["getItem", 18, 0, 0, 4, 1, 149, 0], ["getItemAt", 2, 0, 0, 4, 1, 149, 0]]],
			["NumberFormattingService", 0, 0, "Microsoft.ExcelServices.NumberFormattingService", 4, 0, [], [], [], [["getFormatter", 0, 0, 0, 0, 1, 150, 0]]],
			["NumberFormatter", 0, 0, 0, 0, 0, [["isDateTime", 3, 0, 0, 0], ["isPercent", 3, 0, 0, 0], ["isCurrency", 3, 0, 0, 0], ["isNumeric", 3, 0, 0, 0], ["isText", 3, 0, 0, 0], ["hasYear", 3, 0, 0, 0], ["hasMonth", 3, 0, 0, 0], ["hasDayOfWeek", 3, 0, 0, 0]], [], [["format", 0, 0, 0, 0, 1]]],
			["CustomFunctionManager", 4, 0, "Microsoft.ExcelServices.CustomFunctionManager", 4, 0, [["status", 2, 0, 0, 0]], [], [["register", 0, 0, 0, 0, 2]]],
			["Style", 0, 0, 0, 0, 0, [["builtIn", 3, 0, 0, 0], ["formulaHidden", 1, 0, 0, 0], ["horizontalAlignment", 1, 0, 0, 0], ["includeAlignment", 1, 0, 0, 0], ["includeBorder", 1, 0, 0, 0], ["includeFont", 1, 0, 0, 0], ["includeNumber", 1, 0, 0, 0], ["includePatterns", 1, 0, 0, 0], ["includeProtection", 1, 0, 0, 0], ["indentLevel", 1, 0, 0, 0], ["locked", 1, 0, 0, 0], ["name", 3, 0, 0, 0], ["numberFormat", 1, 0, 0, 0], ["numberFormatLocal", 1, 0, 0, 0], ["readingOrder", 1, 0, 0, 0], ["shrinkToFit", 1, 0, 0, 0], ["verticalAlignment", 1, 0, 0, 0], ["wrapText", 1, 0, 0, 0], ["textOrientation", 5, 1, 0, 0], ["autoIndent", 1, 1, 0, 0]], [["borders", 51, 0, 0, 4, 0, 64], ["font", 35, 0, 0, 4, 0, 63], ["fill", 35, 0, 0, 4, 0, 62]], [["delete", 0, 0, 0, 0, 0]]],
			["StyleCollection", 1, 0, 0, 0, 151, [], [], [["add", 0, 0, 0, 0, 1], ["getCount", 2, 2, 0, 4, 0]], [["getItem", 18, 0, 0, 4, 1, 151, 0], ["getItemAt", 2, 2, 0, 4, 1, 151, 0]]],
			["TableStyleCollection", 1, 0, 0, 0, 58, [], [], [["getCount", 2, 0, 0, 4, 0], ["setDefault", 0, 0, 0, 0, 1]], [["getItem", 18, 0, 0, 4, 1, 58, 0], ["getItemOrNullObject", 2, 0, 0, 4, 1, 58, 0], ["add", 8, 0, 0, 0, 2, 58, 0], ["getDefault", 0, 0, 0, 0, 0, 58, 0]]],
			["TableStyle", 0, 0, 0, 0, 0, [["name", 1, 0, 0, 0], ["readOnly", 3, 0, 0, 0], ["_Id", 2, 0, 0, 0]], [], [["delete", 0, 0, 0, 0, 0]], [["duplicate", 0, 0, 0, 0, 0, 58, 0]]],
			["PivotTableStyleCollection", 1, 0, 0, 0, 121, [], [], [["getCount", 2, 0, 0, 4, 0], ["setDefault", 0, 0, 0, 0, 1]], [["getItem", 18, 0, 0, 4, 1, 121, 0], ["getItemOrNullObject", 2, 0, 0, 4, 1, 121, 0], ["add", 8, 0, 0, 0, 2, 121, 0], ["getDefault", 0, 0, 0, 0, 0, 121, 0]]],
			["PivotTableStyle", 0, 0, 0, 0, 0, [["name", 1, 0, 0, 0], ["readOnly", 3, 0, 0, 0], ["_Id", 2, 0, 0, 0]], [], [["delete", 0, 0, 0, 0, 0]], [["duplicate", 0, 0, 0, 0, 0, 121, 0]]],
			["SlicerStyleCollection", 1, 0, 0, 0, 152, [], [], [["getCount", 2, 0, 0, 4, 0], ["setDefault", 0, 0, 0, 0, 1]], [["getItem", 18, 0, 0, 4, 1, 152, 0], ["getItemOrNullObject", 2, 0, 0, 4, 1, 152, 0], ["add", 8, 0, 0, 0, 2, 152, 0], ["getDefault", 0, 0, 0, 0, 0, 152, 0]]],
			["SlicerStyle", 0, 0, 0, 0, 0, [["name", 1, 0, 0, 0], ["readOnly", 3, 0, 0, 0], ["_Id", 2, 0, 0, 0]], [], [["delete", 0, 0, 0, 0, 0]], [["duplicate", 0, 0, 0, 0, 0, 152, 0]]],
			["TimelineStyleCollection", 1, 0, 0, 0, 153, [], [], [["getCount", 2, 0, 0, 4, 0], ["setDefault", 0, 0, 0, 0, 1]], [["getItem", 18, 0, 0, 4, 1, 153, 0], ["getItemOrNullObject", 2, 0, 0, 4, 1, 153, 0], ["add", 8, 0, 0, 0, 2, 153, 0], ["getDefault", 0, 0, 0, 0, 0, 153, 0]]],
			["TimelineStyle", 0, 0, 0, 0, 0, [["name", 1, 0, 0, 0], ["readOnly", 3, 0, 0, 0], ["_Id", 2, 0, 0, 0]], [], [["delete", 0, 0, 0, 0, 0]], [["duplicate", 0, 0, 0, 0, 0, 153, 0]]],
			["InternalTest", 0, 0, "Microsoft.ExcelServices.InternalTest", 4, 0, [], [], [["delay", 0, 0, 0, 0, 1], ["triggerMessage", 0, 5, 0, 0, 4], ["_RegisterTestEvent", 0, 5, 0, 0, 0], ["_UnregisterTestEvent", 0, 5, 0, 0, 0], ["triggerTestEvent", 0, 5, 0, 0, 2], ["triggerPostProcess", 0, 5, 0, 0, 0], ["_RegisterTest1Event", 0, 5, 0, 0, 0], ["_UnregisterTest1Event", 0, 5, 0, 0, 0], ["triggerTestEventWithFilter", 0, 5, 0, 0, 3], ["firstPartyMethod", 2, 5, 0, 5, 0], ["_RegisterCustomFunctionExecutionBeginEvent", 0, 12, 0, 0, 0], ["_UnregisterCustomFunctionExecutionBeginEvent", 0, 12, 0, 0, 0], ["_RegisterCustomFunctionExecutionEndEvent", 0, 12, 0, 0, 0], ["_UnregisterCustomFunctionExecutionEndEvent", 0, 12, 0, 0, 0], ["unregisterAllCustomFunctionExecutionEvents", 0, 12, 0, 0, 0], ["saveWorkbookToTempFile", 2, 15, 0, 0, 1], ["compareTempFilesAreIdentical", 2, 15, 0, 0, 2], ["updateRangeValueOnCurrentSheet", 0, 15, 0, 2, 2], ["triggerUserUndo", 2, 15, 0, 0, 0], ["triggerUserRedo", 2, 15, 0, 0, 0], ["enterCellEdit", 0, 2, 0, 0, 1], ["installCustomFunctionsFromCache", 0, 2, 0, 0, 0], ["recalc", 0, 2, 0, 0, 2], ["recalcBySolutionId", 0, 2, 0, 0, 1], ["safeForCellEditModeMethod", 0, 2, 0, 0, 1], ["exitCellEdit", 0, 2, 0, 0, 0], ["noPermissionMethod", 0, 2, 0, 0, 1], ["verifyCustomFunctionListExist", 0, 2, 0, 0, 0]], [], [["CustomFunctionExecutionBeginEvent", 0, 12, "MessageType.customFunctionExecutionBeginEvent", "", "_RegisterCustomFunctionExecutionBeginEvent", "_UnregisterCustomFunctionExecutionBeginEvent"], ["CustomFunctionExecutionEndEvent", 0, 12, "MessageType.customFunctionExecutionEndEvent", "", "_RegisterCustomFunctionExecutionEndEvent", "_UnregisterCustomFunctionExecutionEndEvent"], ["Test1Event", 1, 5, "MessageType.test1Event", "", "_RegisterTest1Event", "_UnregisterTest1Event"], ["TestEvent", 1, 5, "MessageType.testEvent", "", "_RegisterTestEvent", "_UnregisterTestEvent"]]],
			["PageLayout", 0, 0, 0, 0, 0, [["orientation", 1, 0, 0, 0], ["paperSize", 1, 0, 0, 0], ["blackAndWhite", 1, 0, 0, 0], ["printErrors", 1, 0, 0, 0], ["zoom", 1, 0, 0, 0], ["centerHorizontally", 1, 0, 0, 0], ["centerVertically", 1, 0, 0, 0], ["printHeadings", 1, 0, 0, 0], ["printGridlines", 1, 0, 0, 0], ["leftMargin", 1, 0, 0, 0], ["rightMargin", 1, 0, 0, 0], ["topMargin", 1, 0, 0, 0], ["bottomMargin", 1, 0, 0, 0], ["headerMargin", 1, 0, 0, 0], ["footerMargin", 1, 0, 0, 0], ["printComments", 1, 0, 0, 0], ["draftMode", 1, 0, 0, 0], ["firstPageNumber", 1, 0, 0, 0], ["printOrder", 1, 0, 0, 0]], [["headersFooters", 35, 0, 0, 4, 0, 154]], [["setPrintMargins", 0, 0, 0, 0, 2], ["setPrintArea", 0, 0, 0, 0, 1], ["setPrintTitleRows", 0, 0, 0, 0, 1], ["setPrintTitleColumns", 0, 0, 0, 0, 1]], [["getPrintArea", 10, 0, 0, 4, 0, 29, 0], ["getPrintAreaOrNullObject", 10, 0, 0, 4, 0, 29, 0], ["getPrintTitleRows", 10, 0, 0, 4, 0, 27, 0], ["getPrintTitleRowsOrNullObject", 10, 0, 0, 4, 0, 27, 0], ["getPrintTitleColumns", 10, 0, 0, 4, 0, 27, 0], ["getPrintTitleColumnsOrNullObject", 10, 0, 0, 4, 0, 27, 0]]],
			["HeaderFooter", 0, 0, 0, 0, 0, [["leftHeader", 1, 0, 0, 0], ["centerHeader", 1, 0, 0, 0], ["rightHeader", 1, 0, 0, 0], ["leftFooter", 1, 0, 0, 0], ["centerFooter", 1, 0, 0, 0], ["rightFooter", 1, 0, 0, 0]]],
			["HeaderFooterGroup", 0, 0, 0, 0, 0, [["state", 1, 0, 0, 0], ["useSheetMargins", 1, 0, 0, 0], ["useSheetScale", 1, 0, 0, 0]], [["defaultForAllPages", 35, 0, 0, 4, 0, 155], ["firstPage", 35, 0, 0, 4, 0, 155], ["evenPages", 35, 0, 0, 4, 0, 155], ["oddPages", 35, 0, 0, 4, 0, 155]]],
			["PageBreak", 0, 0, 0, 0, 0, [["_Id", 2, 0, 0, 0], ["columnIndex", 3, 0, 0, 0], ["rowIndex", 3, 0, 0, 0]], [], [["delete", 0, 0, 0, 0, 0]], [["getCellAfterBreak", 10, 0, 0, 4, 0, 27, 0]]],
			["PageBreakCollection", 1, 0, 0, 0, 156, [], [], [["removePageBreaks", 0, 0, 0, 0, 0], ["getCount", 2, 0, 0, 4, 0]], [["getItem", 18, 0, 0, 4, 1, 156, 0], ["add", 8, 0, 0, 0, 1, 156, 0]]],
			["DataConnectionCollection", 0, 0, 0, 0, 0, [], [], [["refreshAll", 0, 0, 0, 0, 0]]],
			["RangeCollection", 1, 0, 0, 0, 27, [], [], [["getCount", 2, 0, 0, 4, 0]], [["getItemAt", 2, 0, 0, 4, 1, 27, 0]]],
			["CommentCollection", 1, 0, 0, 0, 157, [], [], [["getCount", 2, 0, 0, 4, 0]], [["getItem", 18, 0, 0, 4, 1, 157, 0], ["getItemAt", 2, 0, 0, 4, 1, 157, 0], ["add", 8, 0, 0, 0, 3, 157, 0], ["getItemByReplyId", 2, 0, 0, 4, 1, 157, 0], ["getItemByCell", 2, 0, 0, 4, 1, 157, 0]]],
0, 0, 4, 0, 166], ["lineFormat", 35, 0, 0, 4, 0, 167]], [["setZOrder", 0, 0, 0, 0, 1], ["incrementLeft", 0, 0, 0, 0, 1], ["incrementTop", 0, 0, 0, 0, 1], ["incrementRotation", 0, 0, 0, 0, 1], ["scaleHeight", 0, 0, 0, 0, 3], ["scaleWidth", 0, 0, 0, 0, 3], ["delete", 0, 0, 0, 0, 0], ["getAsImage", 0, 0, 0, 0, 1], ["_RegisterActivatedEvent", 0, 0, 0, 0, 0], ["_UnregisterActivatedEvent", 0, 0, 0, 0, 0], ["_RegisterDeactivatedEvent", 0, 0, 0, 0, 0], ["_UnregisterDeactivatedEvent", 0, 0, 0, 0, 0]], [["copyTo", 8, 11, 0, 0, 1, 160, "_GetShapeById"], ["_GetShapeById", 2, 0, 0, 4, 1, 160, 0]], [["Activated", 0, 0, "MessageType.shapeActivatedEvent", "this.id", "_RegisterActivatedEvent", "_UnregisterActivatedEvent"], ["Deactivated", 0, 0, "MessageType.shapeDeactivatedEvent", "this.id", "_RegisterDeactivatedEvent", "_UnregisterDeactivatedEvent"]]],
			["GeometricShape", 0, 0, 0, 0, 0, [["id", 3, 0, 0, 0]], [["shape", 2, 0, 0, 4, 0, 160]]],
			["Image", 0, 0, 0, 0, 0, [["id", 3, 0, 0, 0], ["format", 3, 0, "format", 0]], [["shape", 2, 0, 0, 4, 0, 160]]],
			["ShapeGroup", 0, 0, 0, 0, 0, [["id", 3, 0, 0, 0]], [["shapes", 19, 0, 0, 4, 0, 168], ["shape", 2, 0, 0, 4, 0, 160]], [["ungroup", 0, 0, 0, 0, 0]]],
			["GroupShapeCollection", 1, 0, 0, 0, 160, [], [], [["getCount", 2, 0, 0, 4, 0]], [["_GetItem", 18, 0, 0, 4, 1, 160, 0], ["getItemAt", 2, 0, 0, 4, 1, 160, 0], ["getItem", 2, 0, 0, 4, 1, 160, 0]]],
			["Line", 0, 0, 0, 0, 0, [["id", 3, 0, 0, 0], ["connectorType", 1, 0, "connectorType", 0], ["beginArrowheadLength", 1, 0, 0, 0], ["beginArrowheadStyle", 1, 0, 0, 0], ["beginArrowheadWidth", 1, 0, 0, 0], ["endArrowheadLength", 1, 0, 0, 0], ["endArrowheadStyle", 1, 0, 0, 0], ["endArrowheadWidth", 1, 0, 0, 0], ["isBeginConnected", 3, 0, 0, 0], ["beginConnectedSite", 3, 0, 0, 0], ["isEndConnected", 3, 0, 0, 0], ["endConnectedSite", 3, 0, 0, 0]], [["shape", 2, 0, 0, 4, 0, 160], ["beginConnectedShape", 2, 0, 0, 4, 0, 160], ["endConnectedShape", 2, 0, 0, 4, 0, 160]], [["connectBeginShape", 0, 0, 0, 0, 2], ["disconnectBeginShape", 0, 0, 0, 0, 0], ["connectEndShape", 0, 0, 0, 0, 2], ["disconnectEndShape", 0, 0, 0, 0, 0]]],
			["ShapeFill", 0, 0, 0, 0, 0, [["foregroundColor", 1, 0, 0, 0], ["type", 3, 0, 0, 0], ["transparency", 1, 0, 0, 0]], [], [["clear", 0, 0, 0, 0, 0], ["setSolidColor", 0, 0, 0, 0, 1]]],
			["ShapeLineFormat", 0, 0, 0, 0, 0, [["visible", 1, 0, 0, 0], ["color", 1, 0, 0, 0], ["style", 1, 0, 0, 0], ["weight", 1, 0, 0, 0], ["dashStyle", 1, 0, 0, 0], ["transparency", 1, 0, 0, 0]]],
			["TextFrame", 0, 0, 0, 0, 0, [["leftMargin", 1, 0, 0, 0], ["rightMargin", 1, 0, 0, 0], ["topMargin", 1, 0, 0, 0], ["bottomMargin", 1, 0, 0, 0], ["horizontalAlignment", 1, 0, 0, 0], ["horizontalOverflow", 1, 0, 0, 0], ["verticalAlignment", 1, 0, 0, 0], ["verticalOverflow", 1, 0, 0, 0], ["orientation", 1, 0, 0, 0], ["readingOrder", 1, 0, 0, 0], ["hasText", 3, 0, 0, 0], ["autoSizeSetting", 1, 0, 0, 0]], [["textRange", 2, 0, 0, 4, 0, 169]], [["deleteText", 0, 0, 0, 0, 0]]],
			["TextRange", 0, 0, 0, 0, 0, [["text", 1, 0, 0, 0]], [["font", 35, 0, 0, 4, 0, 170]], [], [["getSubstring", 0, 0, 0, 0, 2, 169, 0]]],
			["ShapeFont", 0, 0, 0, 0, 0, [["size", 1, 0, 0, 0], ["name", 1, 0, 0, 0], ["color", 1, 0, 0, 0], ["bold", 1, 0, 0, 0], ["italic", 1, 0, 0, 0], ["underline", 1, 0, 0, 0]]],
			["Slicer", 0, 0, 0, 0, 0, [["id", 3, 0, 0, 0], ["name", 1, 0, 0, 0], ["caption", 1, 0, 0, 0], ["left", 1, 0, 0, 0], ["top", 1, 0, 0, 0], ["width", 1, 0, 0, 0], ["height", 1, 0, 0, 0], ["nameInFormula", 1, 3, 0, 0], ["isFilterCleared", 3, 0, 0, 0], ["style", 1, 0, 0, 0], ["sortBy", 1, 0, 0, 0], ["sortUsingCustomLists", 1, 15, 0, 0], ["columnCount", 1, 15, 0, 0], ["disableMoveResizeUI", 1, 15, 0, 0], ["displayHeader", 1, 15, 0, 0], ["rowHeight", 1, 15, 0, 0]], [["slicerItems", 19, 0, 0, 4, 0, 171], ["worksheet", 35, 0, 0, 4, 0, 40], ["slicerStyle", 35, 15, 0, 4, 0, 152]], [["delete", 0, 0, 0, 0, 0], ["clearFilters", 0, 0, 0, 0, 0], ["getSelectedItems", 0, 0, 0, 0, 0], ["selectItems", 0, 0, 0, 0, 1], ["activate", 2, 15, 0, 0, 0], ["setStyle", 0, 15, 0, 0, 1]]],
			["SlicerCollection", 1, 0, 0, 0, 30, [], [], [["getCount", 2, 0, 0, 4, 0]], [["add", 8, 0, 0, 0, 3, 30, 0], ["getItemAt", 2, 0, 0, 4, 1, 30, 0], ["getItem", 18, 0, 0, 4, 1, 30, 0], ["getItemOrNullObject", 2, 0, 0, 4, 1, 30, 0]]],
			["SlicerItem", 0, 0, 0, 0, 0, [["key", 3, 0, 0, 0], ["name", 3, 0, 0, 0], ["isSelected", 1, 0, 0, 0], ["hasData", 3, 0, 0, 0]]],
