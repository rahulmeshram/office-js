...
Office.onReady(function (hostInfo) {
    // Do a check for the host (partially to avoid runtime exceptions if loading the page in a web brwoser)
    if (hostInfo.host !== Office.HostType.Excel) {
...
// First of all, enqueue the substitution once the host *is* ready:
Office.onReady(function (info) {
    window.OfficeRuntime = getOfficeRuntimeImplementation(info);
});
// But in the meantime, create an OfficeRuntime variable that exposes the right signatures,
//    but will hold off execution until the host functionality is exposed.
window.OfficeRuntime = {
    AsyncStorage: createAsyncStorageWrapper(),
    displayWebDialog: wrapMethod(function (impl) { return impl.displayWebDialog; })
};
...
function createAsyncStorageWrapper() {
    return {
    ...
    function wrapStorageMethod(methodName) {
        return wrapMethod(function (impl) { return impl.AsyncStorage[methodName]; });
    }
}
function wrapMethod(methodFetcher) {
    return function () {
        var _this = this;
        var args = arguments;
        return Office.onReady().then(function (info) { return methodFetcher(getOfficeRuntimeImplementation(info)).apply(_this, args); });
    };
}
...
/***/ })
/******/ ]);
//# sourceMappingURL=office-runtime-load-wrapper.g.js.map